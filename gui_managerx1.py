...
class DiskDirectoryPanel(tk.Frame):
    ...
    def find_files(self):
        ...
        try:
            ...
            search_window.title("🔍 Dosya Arama - Wildcard Desteği")
            ...
            # Arama dizini seçimi
            tk.Label(control_frame, text="📂 Arama Dizini:", bg=ModernStyle.BG_SECONDARY, 
                    fg=ModernStyle.FG_PRIMARY, font=("Arial", 9, "bold")).pack(side=tk.LEFT)
            
            self.search_dir_var = tk.StringVar(value=self.last_search_directory)
            dir_entry = tk.Entry(control_frame, textvariable=self.search_dir_var, width=40, font=("Arial", 9))
            dir_entry.pack(side=tk.LEFT, padx=(5, 5))
            
            tk.Button(control_frame, text="📁", command=lambda: self.select_search_directory(search_window), 
                     font=("Arial", 8), width=3).pack(side=tk.LEFT, padx=(0, 10))
            
            # Arama kontrolleri - İkinci satır
            search_frame = tk.Frame(main_frame, bg=ModernStyle.BG_SECONDARY)
            search_frame.pack(fill=tk.X, pady=(0, 10))
            
            tk.Label(search_frame, text="🔍 Arama Kalıbı:", bg=ModernStyle.BG_SECONDARY, 
                    fg=ModernStyle.FG_PRIMARY, font=("Arial", 9, "bold")).pack(side=tk.LEFT)
            
            self.search_pattern_var = tk.StringVar(value="*.d64")
            pattern_entry = tk.Entry(search_frame, textvariable=self.search_pattern_var, width=20, font=("Arial", 9))
            pattern_entry.pack(side=tk.LEFT, padx=(5, 5))
            
            # Butonlar
            self.search_active = False
            tk.Button(search_frame, text="🔍 Ara", command=lambda: self.start_search(search_window), 
                     font=("Arial", 8), width=8, bg="#4CAF50", fg="white").pack(side=tk.LEFT, padx=2)
            
            self.stop_btn = tk.Button(search_frame, text="⏹️ Durdur", command=self.stop_search, 
                                     font=("Arial", 8), width=8, bg="#f44336", fg="white", state="disabled")
            self.stop_btn.pack(side=tk.LEFT, padx=2)
            
            self.select_btn = tk.Button(search_frame, text="✅ Seç", command=lambda: self.select_found_file(search_window), 
                                       font=("Arial", 8), width=8, bg="#2196F3", fg="white", state="disabled")
            self.select_btn.pack(side=tk.LEFT, padx=2)
            
            # Yardım metni
            help_frame = tk.Frame(main_frame, bg=ModernStyle.BG_SECONDARY)
            help_frame.pack(fill=tk.X, pady=(0, 10))
            
            help_text = "💡 Wildcard kullanımı: * (herhangi), ? (tek karakter), *.d64, test*.prg, game?.t64"
            tk.Label(help_frame, text=help_text, bg=ModernStyle.BG_SECONDARY, 
                    fg=ModernStyle.FG_ACCENT, font=("Arial", 8)).pack(side=tk.LEFT)
