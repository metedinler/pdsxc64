...
class DiskDirectoryPanel(tk.Frame):
    ...
    def setup_ui(self):
        ...
        # TreeView font style - 1 punto küçültüldü
        style = ttk.Style()
        style.configure("Custom.Treeview", font=("Arial", 8))
        style.configure("Custom.Treeview.Heading", font=("Arial", 8, "bold"))
        
        self.tree = ttk.Treeview(tree_frame, columns=columns, show="headings", height=15, style="Custom.Treeview")
        ...
        tk.Button(analysis_frame, text="🚫 Illegal", command=self.analyze_illegal_opcodes, font=("Arial", 7), height=1, width=10).pack(side=tk.LEFT, padx=2)
    ...
    def on_file_select(self, event):
        """Dosya seçildiğinde - DETAYLI LOG SİSTEMİ + OTOMATİK HYBRİD ANALİZ"""
        ...
        if selection:
            ...
            if self.parent_gui and self.selected_entry:
                ...
                # ✅ OTOMATİK HYBRİD ANALİZ - Dosya seçildikten sonra
                self.log_to_terminal_and_file("🔄 META ANALİZ SONRASI OTOMATİK HYBRİD ANALİZ BAŞLATILIYOR...", "INFO")
                print("🔄 Otomatik Hybrid Analiz başlatılıyor...")
                
                # Threading ile hybrid analiz
                threading.Thread(target=self._auto_hybrid_analysis_thread, 
                               args=(self.selected_entry,), daemon=True).start()
    
    def _auto_hybrid_analysis_thread(self, entry):
        """Otomatik hybrid analiz thread'i - DOSYA SEÇİMİ SONRASI"""
        try:
            self.root.after(0, lambda: self.log_to_terminal_and_file("🔄 Otomatik hybrid analiz başlatıldı...", "INFO"))
            print("🔄 AUTO HYBRID ANALYSIS - Dosya meta analizi sonrası hybrid analiz")
            
            # PRG verisini çıkar
            prg_data = self.extract_prg_data(entry)
            if not prg_data:
                self.root.after(0, lambda: self.log_to_terminal_and_file("❌ PRG verisi çıkarılamadı - otomatik analiz iptal", "ERROR"))
                return
            
            # Enhanced Universal Disk Reader ile otomatik hybrid analiz
            if analyze_hybrid_program:
                self.root.after(0, lambda: self.log_to_terminal_and_file("✅ Enhanced Universal Disk Reader ile analiz yapılıyor...", "INFO"))
                analysis_result = analyze_hybrid_program(prg_data)
                
                if analysis_result:
                    # Analiz başarılı - sonuçları entry'ye kaydet
                    entry['hybrid_analysis'] = analysis_result
                    
                    # Hybrid program ise otomatik adres belirleme
                    if analysis_result.get("is_hybrid"):
                        basic_info = analysis_result.get('basic_info', {})
                        assembly_info = analysis_result.get('assembly_info', {})
                        
                        entry['basic_start'] = basic_info.get('start_address', 0x0801)
                        entry['basic_end'] = basic_info.get('end_address', 0x0801)
                        entry['assembly_start'] = assembly_info.get('start_address', 0)
                        entry['assembly_end'] = assembly_info.get('end_address', 0)
                        
                        success_msg = f"✅ HYBRID PROGRAM BELİRLENDİ"
                        self.root.after(0, lambda: self.log_to_terminal_and_file(success_msg, "SUCCESS"))
                        self.root.after(0, lambda: self.log_to_terminal_and_file(f"   📝 BASIC: ${entry['basic_start']:04X} - ${entry['basic_end']:04X}", "INFO"))
                        self.root.after(0, lambda: self.log_to_terminal_and_file(f"   ⚙️ ASSEMBLY: ${entry['assembly_start']:04X} - ${entry['assembly_end']:04X}", "INFO"))
                        
                        print(f"✅ HYBRID DETECTED:")
                        print(f"   BASIC: ${entry['basic_start']:04X} - ${entry['basic_end']:04X}")
                        print(f"   ASSEMBLY: ${entry['assembly_start']:04X} - ${entry['assembly_end']:04X}")
                    else:
                        self.root.after(0, lambda: self.log_to_terminal_and_file("📋 Pure program türü tespit edildi (hybrid değil)", "INFO"))
                        
                    # TreeView'daki entry'yi güncelle - program türünü hybrid bilgisiyle
                    program_type = analysis_result.get('program_type', 'UNKNOWN')
                    if analysis_result.get("is_hybrid"):
                        program_type = f"🔀HYBRID({program_type})"
                    
                    # TreeView güncelle
                    self.root.after(0, lambda: self._update_treeview_program_type(entry, program_type))
                    
                else:
                    self.root.after(0, lambda: self.log_to_terminal_and_file("⚠️ Hybrid analiz sonuç döndürmedi", "WARNING"))
                    
            else:
                self.root.after(0, lambda: self.log_to_terminal_and_file("❌ Enhanced Universal Disk Reader bulunamadı", "ERROR"))
                
        except Exception as e:
            error_msg = f"Otomatik hybrid analiz hatası: {e}"
            self.root.after(0, lambda: self.log_to_terminal_and_file(error_msg, "ERROR"))
            print(f"❌ AUTO HYBRID ANALYSIS ERROR: {e}")
    
    def _update_treeview_program_type(self, entry, new_program_type):
        """TreeView'da seçili entry'nin program türünü güncelle"""
        try:
            filename = entry.get('filename', '')
            if not filename:
                return
                
            # TreeView'da dosyayı bul ve güncelle
            for item in self.tree.get_children():
                item_values = list(self.tree.item(item)['values'])
                if len(item_values) > 0 and item_values[0] == filename:
                    # Program type column (index 4) güncelle
                    if len(item_values) > 4:
                        item_values[4] = new_program_type
                        self.tree.item(item, values=item_values)
                        self.log_to_terminal_and_file(f"📋 TreeView güncellendi: {filename} -> {new_program_type}", "INFO")
                        break
                        
        except Exception as e:
            self.log_to_terminal_and_file(f"TreeView güncelleme hatası: {e}", "ERROR")
    
    def extract_prg_data(self, entry):
        """PRG verisi çıkar - DiskDirectoryPanel için özel method"""
        try:
            # Eğer entry'de zaten prg_data varsa direkt kullan
            if 'prg_data' in entry:
                return entry['prg_data']
            
            # Kaynak dosyadan oku - parent GUI'den al
            if self.parent_gui:
                return self.parent_gui.extract_prg_data(entry)
            else:
                self.log_to_terminal_and_file("Parent GUI bulunamadı - PRG verisi çıkarılamadı", "ERROR")
                return None
                
        except Exception as e:
            self.log_to_terminal_and_file(f"PRG verisi çıkarma hatası: {e}", "ERROR")
            return None
    ...
    def show_processed_files(self):
        """İşlenmiş dosyaları Excel tarzı grid view'da göster"""
        # Çıktı klasörlerini kontrol et
        output_dirs = ["asm_files", "c_files", "qbasic_files", "pdsx_files", "pseudo_files", "basic_files"]
        found_files = []
        
        # Dosyaları topla ve analiz et
        for output_dir in output_dirs:
            if os.path.exists(output_dir):
                for file in os.listdir(output_dir):
                    file_path = os.path.join(output_dir, file)
                    if os.path.isfile(file_path):
                        file_info = self._analyze_processed_file(file_path, output_dir)
                        found_files.append(file_info)
        
        # Excel tarzı grid view penceresi
        proc_window = debug_toplevel(self.parent_gui.root if self.parent_gui else None, title="İşlenmiş Dosyalar - Excel Tarzı Görünüm")
        proc_window.geometry("1000x600")
        proc_window.configure(bg='white')
        
        # Ana frame
        main_frame = debug_frame(proc_window, debug_name="Processed Files Main Frame")
        main_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Başlık ve araç çubuğu
        toolbar_frame = debug_frame(main_frame, debug_name="Processed Files Toolbar")
        toolbar_frame.pack(fill=tk.X, pady=(0, 5))
        
        debug_label(toolbar_frame, debug_name="Processed Files Title", 
                   text=f"📋 İşlenmiş Dosyalar ({len(found_files)} adet)", 
                   font=('Arial', 12, 'bold')).pack(side=tk.LEFT)
        
        # Excel export butonları
        export_frame = debug_frame(toolbar_frame, debug_name="Export Buttons Frame")
        export_frame.pack(side=tk.RIGHT)
        
        debug_button(export_frame, debug_name="Export Excel", text="📊 Excel'e Aktar", 
                    command=lambda: self._export_to_excel(found_files),
                    bg='#2E8B57', fg='white', font=('Arial', 9)).pack(side=tk.LEFT, padx=2)
        
        debug_button(export_frame, debug_name="Export CSV", text="📄 CSV'ye Aktar", 
                    command=lambda: self._export_to_csv(found_files),
                    bg='#4169E1', fg='white', font=('Arial', 9)).pack(side=tk.LEFT, padx=2)
        
        debug_button(export_frame, debug_name="Refresh Files", text="🔄 Yenile", 
                    command=lambda: self._refresh_processed_files(proc_window),
                    bg='#FF8C00', fg='white', font=('Arial', 9)).pack(side=tk.LEFT, padx=2)
        
        # Treeview için frame
        tree_frame = debug_frame(main_frame, debug_name="Processed Files Tree Frame")
        tree_frame.pack(fill=tk.BOTH, expand=True)
        
        # Excel tarzı Treeview sütunları (planınıza göre)
        columns = ('Dosya Adı', 'Tip', 'Kategori', 'Boyut', 'Oluşturma Tarihi', 'Son Değiştirme', 
                  'Başlangıç Adresi', 'Program Türü', 'BASIC Boyutu', 'Assembly Başlangıcı', 'Kaynak Disk')
        
        tree = ttk.Treeview(tree_frame, columns=columns, show='headings', height=20)
        
        # Sütun başlıkları ve genişlikleri (Excel tarzı)
        column_widths = {
            'Dosya Adı': 150, 'Tip': 60, 'Kategori': 80, 'Boyut': 80, 'Oluşturma Tarihi': 120,
            'Son Değiştirme': 120, 'Başlangıç Adresi': 100, 'Program Türü': 90, 
            'BASIC Boyutu': 90, 'Assembly Başlangıcı': 120, 'Kaynak Disk': 120
        }
        ...
        for file_info in found_files:
            tree.insert('', tk.END, values=(
                ...
                file_info['program_type'],
                file_info['basic_size'],
                file_info['asm_start'],
                file_info['source_disk']
            ))
        ...
        # Alt buton çubuğu
        bottom_frame = debug_frame(main_frame, debug_name="Bottom Buttons Frame")
        bottom_frame.pack(fill=tk.X, pady=(5, 0))
        
        debug_button(bottom_frame, debug_name="Open File", text="📂 Seçili Dosyayı Aç", 
                    command=lambda: self._open_selected_file(tree),
                    bg='#228B22', fg='white').pack(side=tk.LEFT, padx=5)
        
        debug_button(bottom_frame, debug_name="File Properties", text="🔍 Özellikler", 
                    command=lambda: self._show_file_properties(tree),
                    bg='#4682B4', fg='white').pack(side=tk.LEFT, padx=5)
        
        debug_button(bottom_frame, debug_name="Delete File", text="🗑️ Sil", 
                    command=lambda: self._delete_selected_file(tree),
                    bg='#DC143C', fg='white').pack(side=tk.LEFT, padx=5)
        
        debug_button(bottom_frame, debug_name="Close Window", text="❌ Kapat", 
                    command=proc_window.destroy).pack(side=tk.RIGHT, padx=5)
        
        # Excel tarzı alternating row colors
        tree.tag_configure('evenrow', background='#f0f0f0')
        tree.tag_configure('oddrow', background='white')
        
        # Satır renklerini uygula
        for i, item in enumerate(tree.get_children()):
            tree.set(item, tags=('evenrow' if i % 2 == 0 else 'oddrow',))
        
        # Çift tıklama ile dosya açma
        tree.bind('<Double-1>', lambda e: self._open_selected_file(tree))
        
        if not found_files:
            # Boş durum mesajı
            empty_label = debug_label(tree_frame, debug_name="Empty State", 
                                    text="📂 Henüz işlenmiş dosya bulunmuyor.\n\nDisk imajlarını analiz ettikten sonra sonuçlar burada görünecek.", 
                                    justify=tk.CENTER, font=('Arial', 11))
            empty_label.place(relx=0.5, rely=0.5, anchor=tk.CENTER)
        
        proc_window.focus_set()
        proc_window.grab_set()  # Modal pencere
...
class DisassemblyPanel(tk.Frame):
    ...
    def setup_ui(self):
        ...
        # Assembly formatları listesi - Kısaltılmış
        self.assembly_formats = {
            'tass': 'TASS',
            'kickass': 'KickAss', 
            'dasm': 'DASM',
            'acme': 'ACME',
        ...
        # Format seçim ve Disassembler butonları - aynı satırda
        disasm_row1 = debug_frame(group1, debug_name="Disasm Row 1", bg=ModernStyle.BG_SECONDARY)
        disasm_row1.pack(side=tk.TOP)
        
        # Format combobox - Disassembler'ın solunda
        self.selected_format = tk.StringVar(value='DASM')
        format_combo = ttk.Combobox(disasm_row1, textvariable=self.selected_format,
                                   values=list(self.assembly_formats.values()),
                                   state="readonly", width=10, font=("Arial", 8))
        format_combo.pack(side=tk.LEFT, padx=2)
        
        debug_button(disasm_row1, debug_name="Disassembler", text="Disassembler", command=lambda: self.convert_format('assembly')).pack(side=tk.LEFT, padx=1)
        debug_button(disasm_row1, debug_name="Advanced", text="Advanced", command=lambda: self.convert_format('advanced_disasm')).pack(side=tk.LEFT, padx=1)
        debug_button(disasm_row1, debug_name="Improved", text="Improved", command=lambda: self.convert_format('improved_disasm')).pack(side=tk.LEFT, padx=1)
        debug_button(disasm_row1, debug_name="Hybrid", text="🔀 Hybrid", command=lambda: self.convert_format('hybrid_disasm')).pack(side=tk.LEFT, padx=1)
        debug_button(disasm_row1, debug_name="py65 Pro", text="py65 Pro", command=lambda: self.convert_format('py65_pro')).pack(side=tk.LEFT, padx=1)
        debug_button(disasm_row1, debug_name="Enhanced", text="⚡ Enhanced", command=lambda: self.convert_format('enhanced_disasm')).pack(side=tk.LEFT, padx=1)
        debug_button(disasm_row1, debug_name="Multi", text="🎯 Multi", command=lambda: self.convert_format('multi_disasm')).pack(side=tk.LEFT, padx=1)
        
        # İkinci satır - Checkbox'lar
        disasm_row2 = debug_frame(group1, debug_name="Disasm Row 2", bg=ModernStyle.BG_SECONDARY)
        disasm_row2.pack(side=tk.TOP, pady=2)
        
        self.use_illegal_opcodes = tk.BooleanVar(value=False)
        illegal_check = tk.Checkbutton(disasm_row2, text="Illegal Opcodes", 
                                     variable=self.use_illegal_opcodes,
                                     bg=ModernStyle.BG_SECONDARY, fg=ModernStyle.FG_PRIMARY,
                                     font=("Arial", 8), selectcolor=ModernStyle.BG_PRIMARY)
        illegal_check.pack(side=tk.LEFT, padx=5)
        
        self.enhanced_analysis = tk.BooleanVar(value=True)
        enhanced_check = tk.Checkbutton(disasm_row2, text="Enhanced Features", 
                                      variable=self.enhanced_analysis,
                                      bg=ModernStyle.BG_SECONDARY, fg=ModernStyle.FG_PRIMARY,
                                      font=("Arial", 8), selectcolor=ModernStyle.BG_PRIMARY)
        ...
        # Özel karakter modu - BASIC detokenizer altında
        char_options_frame = debug_frame(group2, debug_name="Char Options Frame", bg=ModernStyle.BG_SECONDARY)
        char_options_frame.pack(side=tk.TOP, pady=2)
        
        # Özel karakter modu seçimi için değişken
        self.special_char_mode = tk.StringVar(value="numeric")
        
        ttk.Radiobutton(char_options_frame, text="[11][13]", variable=self.special_char_mode, 
                      value="numeric", command=self.update_special_char_mode).pack(side=tk.LEFT, padx=2)
        ttk.Radiobutton(char_options_frame, text="{CLR}{HOME}", variable=self.special_char_mode, 
                      value="color_names", command=self.update_special_char_mode).pack(side=tk.LEFT, padx=2)
        ttk.Radiobutton(char_options_frame, text="\\x0B\\x0D", variable=self.special_char_mode, 
                      value="escaped", command=self.update_special_char_mode).pack(side=tk.LEFT, padx=2)
        ...
        if EnhancedBasicDecompiler:
            ...
            # Üçüncü satır - Eksik Transpiler'lar
            trans_row3 = debug_frame(transpiler_container, debug_name="Transpiler Row 3", bg=ModernStyle.BG_SECONDARY)
            trans_row3.pack(side=tk.TOP)
            debug_button(trans_row3, debug_name="ASM to Rust", text="ASM→Rust", command=lambda: self.convert_format('asm_to_rust')).pack(side=tk.LEFT, padx=1)
            debug_button(trans_row3, debug_name="ASM to C2", text="ASM→C2", command=lambda: self.convert_format('asm_to_c2')).pack(side=tk.LEFT, padx=1)
            debug_button(trans_row3, debug_name="ASM to PDSX", text="ASM→PDSX", command=lambda: self.convert_format('asm_to_pdsx')).pack(side=tk.LEFT, padx=1)
            debug_button(trans_row3, debug_name="ASM to CPP", text="ASM→C++", command=lambda: self.convert_format('asm_to_cpp')).pack(side=tk.LEFT, padx=1)
        ...
        # Options frame kaldırıldı - duplicate illegal opcodes checkbox kaldırıldı
        # Illegal opcodes checkbox sadece disasm_row2'de kalacak
    ...
    def convert_format(self, format_type):
        ...
        if self.parent_gui and hasattr(self.parent_gui, 'selected_entry') and self.parent_gui.selected_entry:
            ...
            if format_type == 'assembly':
                # Seçili format ve ayarları al
                selected_format_text = self.selected_format.get()
                # Format text'inden key'i bul
                format_key = 'dasm'  # default
                for key, value in self.assembly_formats.items():
                    if value == selected_format_text:
                        format_key = key
                        break
                
                # Format type'ı güncelle
                format_type = f"assembly_{format_key}"
                
                # Illegal opcodes ve enhanced analysis ayarlarını parent GUI'ye ilet
                if hasattr(self.parent_gui, 'set_disassembly_options'):
                    self.parent_gui.set_disassembly_options(
                        use_illegal_opcodes=self.use_illegal_opcodes.get(),
                        enhanced_analysis=self.enhanced_analysis.get(),
    ...
    def update_special_char_mode(self):
        """Özel karakter modu güncellemesi"""
        try:
            mode = self.special_char_mode.get()
            self.log_message(f"Özel karakter modu güncellendi: {mode}", "INFO")
            log_gui_event("special_char_mode_update", {"mode": mode})
        except Exception as e:
            self.log_message(f"Özel karakter modu güncellenirken hata: {e}", "ERROR")
    
    def log_message(self, message, level="INFO"):
        """DisassemblyPanel için log mesajı - parent GUI'ye yönlendir"""
        if self.parent_gui and hasattr(self.parent_gui, 'log_message'):
            self.parent_gui.log_message(f"[DisassemblyPanel] {message}", level)
        else:
    ...
    def create_disasm_result_tabs(self):
        ...
        tab_configs = [
            ...
            # BASIC Transpiler (13 adet - YENİLER EKLENDİ)
            ...
            ("ASM→Rust", "asm_to_rust"),
            ("ASM→C2", "asm_to_c2"),
            ("ASM→PDSX", "asm_to_pdsx"),
            ("ASM→C++", "asm_to_cpp")
        ]
    ...
    def update_disasm_tab_result(self, tab_key, code):
        ...
        try:
            # Debug log
            print(f"🔍 Tab güncelleme isteği: {tab_key}")
            print(f"📋 Mevcut tablar: {list(self.result_tabs.keys())}")
            ...
            if tab_key in self.result_tabs:
                ...
                # Debug log
                print(f"✅ Tab güncellendi: {tab_key} - {len(code)} karakter")
                ...
                # Tab'ı aktif et - GÜNCEL LİSTE ile
                tab_configs = [
                # Disassembly Modülleri (7 adet)
                ("⚙️ Assembly", "assembly"),
                ("🚀 Advanced", "advanced_disasm"),
                ...
                for i, (_, key) in enumerate(tab_configs):
                    if key == tab_key:
                        ...
                        print(f"✅ Tab aktif edildi: {tab_key} (index: {i})")
                        break
                else:
                    print(f"❌ Tab bulunamadı: {tab_key}")
                
            else:
                print(f"❌ Tab key bulunamadı result_tabs'da: {tab_key}")
                print(f"📋 Mevcut keys: {list(self.result_tabs.keys())}")
                
        except Exception as e:
            print(f"❌ Tab güncelleme hatası: {e}")
            import traceback
            traceback.print_exc()
...
class D64ConverterGUI:
    ...
    def __init__(self, root=None):
        try:
            ...
            # Disassembly seçenekleri - default değerler
    ...
    def set_disassembly_options(self, use_illegal_opcodes=False, enhanced_analysis=True, output_format='dasm'):
        """Disassembly seçeneklerini ayarla"""
        self.disassembly_options = {
            'use_illegal_opcodes': use_illegal_opcodes,
            'enhanced_analysis': enhanced_analysis,
            'output_format': output_format
        }
        self.log_message(f"🎯 Disassembly ayarları güncellendi: Format={output_format}, Illegal={use_illegal_opcodes}, Enhanced={enhanced_analysis}", "INFO")
    ...
    def _convert_format_thread(self, format_type, target_panel):
        ...
        try:
            ...
            # ASSEMBLY ADRESLERİNİ DOĞRU BELİRLE - Hybrid Analiz Sonuçlarını Kullan
            assembly_start_addr = start_address
            assembly_code_data = code_data
            hybrid_analysis = None
            
            # Hybrid analiz sonuçları varsa kullan
            if hasattr(self.selected_entry, 'get') and self.selected_entry.get('hybrid_analysis'):
                hybrid_analysis = self.selected_entry['hybrid_analysis']
                
                # Assembly formatları için assembly bölümünü kullan
                if format_type in ['assembly', 'advanced_disasm', 'improved_disasm', 'py65_pro', 'enhanced_disasm'] or format_type.startswith('assembly_') or format_type.startswith('asm_to_'):
                    if hybrid_analysis.get('is_hybrid') and 'assembly_info' in hybrid_analysis:
                        asm_info = hybrid_analysis['assembly_info']
                        asm_start = asm_info.get('start_address')
                        asm_end = asm_info.get('end_address')
                        
                        if asm_start and asm_start != start_address:
                            # Assembly başlangıç adresi farklı - doğru bölümü çıkar
                            asm_offset = asm_start - start_address
                            if asm_offset >= 0 and asm_offset < len(code_data):
                                if asm_end and asm_end > asm_start:
                                    asm_length = asm_end - asm_start + 1
                                    assembly_code_data = code_data[asm_offset:asm_offset + asm_length]
                                else:
                                    assembly_code_data = code_data[asm_offset:]
                                assembly_start_addr = asm_start
                                
                                self.root.after(0, lambda: self.log_message(f"🔀 HYBRID: Assembly bölümü kullanılıyor: ${asm_start:04X}-${asm_end:04X if asm_end else 'END'}", "INFO"))
                                print(f"🔀 HYBRID ASSEMBLY: ${asm_start:04X}-${asm_end:04X if asm_end else 'END'} ({len(assembly_code_data)} bytes)")
                            else:
                                self.root.after(0, lambda: self.log_message(f"⚠️ HYBRID: Geçersiz assembly offset: {asm_offset}", "WARNING"))
                
                # BASIC formatları için BASIC bölümünü kullan  
                elif format_type in ['basic', 'petcat', 'dlist'] or format_type.startswith('transpile_') or format_type == 'enhanced_basic':
                    if hybrid_analysis.get('is_hybrid') and 'basic_info' in hybrid_analysis:
                        basic_info = hybrid_analysis['basic_info']
                        basic_start = basic_info.get('start_address', 0x0801)
                        basic_end = basic_info.get('end_address')
                        
                        if basic_end and basic_end > basic_start:
                            basic_length = basic_end - basic_start + 1
                            if basic_start >= start_address:
                                basic_offset = basic_start - start_address
                                if basic_offset < len(code_data):
                                    # BASIC bölümü için PRG data'yı yeniden oluştur
                                    basic_code_data = code_data[basic_offset:basic_offset + basic_length]
                                    # BASIC için load address ekle
                                    prg_data = struct.pack('<H', basic_start) + basic_code_data
                                    assembly_start_addr = basic_start
                                    
                                    self.root.after(0, lambda: self.log_message(f"🔀 HYBRID: BASIC bölümü kullanılıyor: ${basic_start:04X}-${basic_end:04X}", "INFO"))
                                    print(f"🔀 HYBRID BASIC: ${basic_start:04X}-${basic_end:04X} ({len(basic_code_data)} bytes)")
            else:
                self.root.after(0, lambda: self.log_message(f"📋 HYBRID: Analiz yok, full dosya kullanılıyor: ${start_address:04X}", "INFO"))
            ...
            # HER FORMAT İÇİN ÇALIŞSIN - PROGRAM TÜRÜ ÖNEMLI DEĞİL, ANCAK HYBRİD ANALIZ VARSA KULLAN
            ...
            # 🔀 HYBRİD PROGRAM ANALİZ KONTROLÜ - Otomatik analiz sonuçlarını kullan
            hybrid_analysis = None
            if hasattr(self, 'selected_entry') and self.selected_entry:
                hybrid_analysis = self.selected_entry.get('hybrid_analysis')
                if hybrid_analysis:
                    self.root.after(0, lambda: self.log_message(f"🔀 HYBRID ANALIZ SONUÇLARI BULUNDU", "INFO"))
                    self.root.after(0, lambda: self.log_message(f"   📋 Program türü: {hybrid_analysis.get('program_type', 'UNKNOWN')}", "INFO"))
                    self.root.after(0, lambda: self.log_message(f"   🔀 Hybrid: {'✅' if hybrid_analysis.get('is_hybrid') else '❌'}", "INFO"))
                    
                    if hybrid_analysis.get('is_hybrid'):
                        basic_start = self.selected_entry.get('basic_start', 0x0801)
                        basic_end = self.selected_entry.get('basic_end', 0x0801)
                        asm_start = self.selected_entry.get('assembly_start', 0)
                        asm_end = self.selected_entry.get('assembly_end', 0)
                        
                        self.root.after(0, lambda: self.log_message(f"   📝 BASIC bölümü: ${basic_start:04X} - ${basic_end:04X}", "INFO"))
                        self.root.after(0, lambda: self.log_message(f"   ⚙️ ASSEMBLY bölümü: ${asm_start:04X} - ${asm_end:04X}", "INFO"))
                        
                        # HYBRID PROGRAM İÇİN BÖLÜM SEÇİCİ LİFTING
                        if format_type in ['basic', 'petcat', 'c64list', 'enhanced_basic', 'transpile_qbasic', 'transpile_c', 'transpile_cpp', 'transpile_pdsx', 'transpile_python']:
                            # BASIC bölümünü çıkar
                            basic_length = basic_end - basic_start + 1
                            if basic_length > 0 and basic_length <= len(prg_data):
                                prg_data_to_use = prg_data[:basic_length + 2]  # Load address dahil
                                self.root.after(0, lambda: self.log_message(f"   ✂️ HYBRID'den BASIC bölümü çıkarıldı: {len(prg_data_to_use)} bytes", "INFO"))
                                prg_data = prg_data_to_use
                                
                        elif format_type in ['assembly', 'advanced_disasm', 'improved_disasm', 'hybrid_disasm', 'py65_pro', 'enhanced_disasm']:
                            # ASSEMBLY bölümünü çıkar
                            if asm_start > 0 and asm_end > asm_start:
                                asm_offset = asm_start - start_address
                                asm_length = asm_end - asm_start + 1
                                if asm_offset >= 0 and asm_offset + asm_length <= len(code_data):
                                    code_data = code_data[asm_offset:asm_offset + asm_length]
                                    start_address = asm_start
                                    # PRG data güncelle
                                    prg_data = struct.pack('<H', start_address) + code_data
                                    self.root.after(0, lambda: self.log_message(f"   ✂️ HYBRID'den ASSEMBLY bölümü çıkarıldı: {len(code_data)} bytes @ ${start_address:04X}", "INFO"))
                ...
                if format_type == 'basic':
                    ...
                    if EnhancedBasicDecompiler:
                        try:
                            ...
                            result_code = enhanced_decompiler.decompile_program(
                                prg_data=prg_data,
            ...
            # Assembly/Machine code veya diğer formatlar - BRIDGE SYSTEM ENTEGRASYONU
                ...
                self.root.after(0, lambda: self.log_message(f"⚙️ ASSEMBLY DİSASSEMBLER BAŞLATILIYOR", "INFO"))
                self.root.after(0, lambda: self.log_message(f"   🎯 Format: {asm_format.upper()}", "INFO"))
                self.root.after(0, lambda: self.log_message(f"   📍 Start address: ${start_address:04X}", "INFO"))
                self.root.after(0, lambda: self.log_message(f"   📏 Code size: {len(code_data)} bytes", "INFO"))
                
                if AdvancedDisassembler:
                    try:
                        disassembler = AdvancedDisassembler(
                            start_address=assembly_start_addr,  # Hybrid analiz sonucu kullan
                            code=assembly_code_data,           # Doğru assembly bölümü kullan
                            use_py65=self.disassembly_panel.use_py65_disassembler.get() if hasattr(self.disassembly_panel, 'use_py65_disassembler') else False,
                            use_illegal_opcodes=self.disassembly_options.get('use_illegal_opcodes', False),
                            enhanced_analysis=self.disassembly_options.get('enhanced_analysis', True),
                            output_format='asm'
                        )
                        ...
                        # BRIDGE SYSTEM - Assembly format'a göre header/direktifler ekle
                        if asm_format == "acme":
                            result_code = f"; ACME Assembler Format - Bridge System v1.0\n"
                            result_code += f"!to \"output.prg\", cbm\n"
                            result_code += f"* = ${assembly_start_addr:04X}\n\n"
                            result_code += asm_code
                        elif asm_format == "cc65":
                            result_code = f"; CC65 Assembler Format - Bridge System v1.0\n"
                            result_code += f".setcpu \"6502\"\n"
                            result_code += f".org ${assembly_start_addr:04X}\n\n"
                            result_code += asm_code
                        elif asm_format == "dasm":
                            result_code = f"; DASM Assembler Format - Bridge System v1.0\n"
                            result_code += f"        processor 6502\n"
                            result_code += f"        org ${assembly_start_addr:04X}\n\n"
                            result_code += asm_code
                        elif asm_format == "tass":
                            result_code = f"; TASS64 Assembler Format - Bridge System v1.0\n"
                            result_code += f".cpu \"6502\"\n"
                            result_code += f"* = ${assembly_start_addr:04X}\n\n"
                            result_code += asm_code
                        elif asm_format == "kickass":
                            result_code = f"// KickAssembler Format - Bridge System v1.0\n"
                            result_code += f".pc = ${assembly_start_addr:04X} \"Main Program\"\n\n"
                            result_code += asm_code.replace(';', '//')  # KickAss comment style
                        else:
                            # Native format
                            result_code = f"; Native 6502 Assembly - Advanced Disassembler\n"
                            result_code += f"; Start Address: ${assembly_start_addr:04X}\n"
                            result_code += f"; Code Size: {len(assembly_code_data)} bytes\n"
                            if hybrid_analysis and hybrid_analysis.get('is_hybrid'):
                                result_code += f"; Hybrid Program - Assembly Section\n"
                            result_code += f"\n"
                            result_code += asm_code
                            
                        self.root.after(0, lambda: self.log_message(f"   ✅ Assembly disassembly başarılı: {len(result_code)} karakter", "SUCCESS"))
                        print(f"✅ ASSEMBLY DISASSEMBLY BAŞARILI: {asm_format.upper()} - {len(result_code)} karakter")
                        
                    except Exception as asm_error:
                        error_msg = f"Advanced Disassembler exception: {asm_error}"
                        self.root.after(0, lambda: self.log_message(f"   ❌ {error_msg}", "ERROR"))
                        print(f"❌ ASSEMBLY DISASSEMBLY EXCEPTION: {error_msg}")
                        
                        result_code = f"; ASSEMBLY DISASSEMBLER ERROR: {error_msg}\n"
                        result_code += f"; Format: {asm_format}\n"
                        result_code += f"; Start: ${start_address:04X}\n"
                        result_code += f"; Size: {len(code_data)} bytes\n"
                        result_code += f"; Raw data preview:\n"
                        for i in range(0, min(64, len(code_data)), 16):
                            line_data = code_data[i:i+16]
                            hex_str = ' '.join(f'{b:02X}' for b in line_data)
                            result_code += f"; ${start_address+i:04X}: {hex_str}\n"
                else:
                    result_code = f"; ASSEMBLY DISASSEMBLER MODULE ERROR\n"
                    result_code += f"; AdvancedDisassembler modülü bulunamadı\n"
                    result_code += f"; Format: {asm_format}\n"
                    result_code += f"; Start: ${start_address:04X}\n"
                    result_code += f"; Size: {len(code_data)} bytes\n"
                    self.root.after(0, lambda: self.log_message(f"   ❌ AdvancedDisassembler modülü bulunamadı", "ERROR"))
            
            # Diğer disassembly formatları - ENHANCED MODULLER
            elif format_type == 'advanced_disasm':
                self.root.after(0, lambda: self.log_message(f"🚀 ADVANCED DISASSEMBLER BAŞLATILIYOR", "INFO"))
                if AdvancedDisassembler:
                    try:
                        disassembler = AdvancedDisassembler(
                            start_address=assembly_start_addr,  # Hybrid analiz sonucu kullan
                        ...
                        if isinstance(result_code, list):
                            result_code = "\n".join(result_code)
                        self.root.after(0, lambda: self.log_message(f"   ✅ Advanced Disassembler başarılı", "SUCCESS"))
                    except Exception as e:
                        error_msg = f"Advanced Disassembler hatası: {e}"
                        result_code = f"; Advanced Disassembler Error: {e}\n"
                        self.root.after(0, lambda msg=error_msg: self.log_message(f"   ❌ {msg}", "ERROR"))
                else:
                    result_code = f"; Advanced Disassembler modülü bulunamadı\n"
                    
            elif format_type == 'improved_disasm':
                self.root.after(0, lambda: self.log_message(f"⚡ IMPROVED DISASSEMBLER BAŞLATILIYOR", "INFO"))
                # Improved Disassembler çağrısı - eğer varsa
                try:
                    if 'ImprovedDisassembler' in globals():
                        disassembler = ImprovedDisassembler(start_address=start_address, code=code_data)
                        result_code = disassembler.disassemble()
                    else:
                        # Fallback to AdvancedDisassembler
                        if AdvancedDisassembler:
                            disassembler = AdvancedDisassembler(start_address=assembly_start_addr, code=assembly_code_data, output_format='improved')
                            result_code = disassembler.disassemble()
                        else:
                            result_code = f"; Improved Disassembler modülü bulunamadı\n"
                    
                    if isinstance(result_code, list):
                        result_code = "\n".join(result_code)
                    self.root.after(0, lambda: self.log_message(f"   ✅ Improved Disassembler başarılı", "SUCCESS"))
                except Exception as e:
                    error_msg = f"Improved Disassembler hatası: {e}"
                    result_code = f"; Improved Disassembler Error: {e}\n"
                    self.root.after(0, lambda msg=error_msg: self.log_message(f"   ❌ {msg}", "ERROR"))
                    
            elif format_type == 'hybrid_disasm':
                self.root.after(0, lambda: self.log_message(f"🔀 HYBRID DISASSEMBLER BAŞLATILIYOR", "INFO"))
                # Hybrid analiz sonuçlarını kullanarak disassembly
                if hybrid_analysis and hybrid_analysis.get('is_hybrid'):
                    result_code = f"; HYBRID PROGRAM DISASSEMBLY\n"
                    result_code += f"; Program Type: {hybrid_analysis.get('program_type', 'UNKNOWN')}\n\n"
                    
                    # BASIC bölümü
                    if 'basic_info' in hybrid_analysis:
                        basic_info = hybrid_analysis['basic_info']
                        result_code += f"; === BASIC SECTION ===\n"
                        result_code += f"; Start: ${basic_info.get('start_address', 0x0801):04X}\n"
                        result_code += f"; End: ${basic_info.get('end_address', 0x0801):04X}\n"
                        
                        # BASIC detokenize
                        if self.basic_parser:
                            basic_lines = self.basic_parser.detokenize(prg_data)
                            if basic_lines:
                                for line in basic_lines[:10]:  # İlk 10 satır
                                    result_code += f"; {line}\n"
                        result_code += f"\n"
                    
                    # ASSEMBLY bölümü
                    if 'assembly_info' in hybrid_analysis:
                        asm_info = hybrid_analysis['assembly_info']
                        result_code += f"; === ASSEMBLY SECTION ===\n"
                        result_code += f"; Start: ${asm_info.get('start_address', 0):04X}\n"
                        result_code += f"; End: ${asm_info.get('end_address', 0):04X}\n"
                        
                        # Assembly disassemble - sadece assembly kısmını
                        if AdvancedDisassembler and asm_info.get('start_address'):
                            asm_start = asm_info.get('start_address')
                            asm_offset = asm_start - start_address
                            if asm_offset >= 0 and asm_offset < len(code_data):
                                asm_code_data = code_data[asm_offset:]
                                disassembler = AdvancedDisassembler(start_address=asm_start, code=asm_code_data)
                                asm_result = disassembler.disassemble()
                                if isinstance(asm_result, list):
                                    asm_result = "\n".join(asm_result)
                                result_code += asm_result
                        
                    self.root.after(0, lambda: self.log_message(f"   ✅ Hybrid Disassembler başarılı", "SUCCESS"))
                else:
                    # Normal disassembly fallback
                    result_code = f"; HYBRID DISASSEMBLER - No hybrid analysis available\n"
                    result_code += f"; Falling back to standard disassembly\n\n"
                    if AdvancedDisassembler:
                        ...
                        if isinstance(asm_result, list):
                            asm_result = "\n".join(asm_result)
                        result_code += asm_result
                    self.root.after(0, lambda: self.log_message(f"   ⚠️ Hybrid analiz bulunamadı, standard disassembly yapıldı", "WARNING"))
                    
            elif format_type == 'py65_pro':
                self.root.after(0, lambda: self.log_message(f"🎯 PY65 PROFESSIONAL BAŞLATILIYOR", "INFO"))
                # py65 Professional disassembler
                result_code = f"; py65 Professional Disassembler v2.0\n"
                result_code += f"; Enhanced 6502 Analysis with Cycle Counting\n"
                result_code += f"; Address: ${assembly_start_addr:04X}\n"
                result_code += f"; Size: {len(assembly_code_data)} bytes\n\n"
                
                if AdvancedDisassembler:
                    try:
                        disassembler = AdvancedDisassembler(
                            start_address=assembly_start_addr,
                            code=assembly_code_data,
                            use_py65=True,
                            use_illegal_opcodes=True,
                            output_format='py65_pro'
                        )
                        asm_result = disassembler.disassemble()
                        if isinstance(asm_result, list):
                            asm_result = "\n".join(asm_result)
                        result_code += asm_result
                        self.root.after(0, lambda: self.log_message(f"   ✅ py65 Professional başarılı", "SUCCESS"))
                    except Exception as e:
                        result_code += f"; py65 Professional Error: {e}\n"
                        self.root.after(0, lambda: self.log_message(f"   ❌ py65 Professional hatası: {e}", "ERROR"))
                else:
                    result_code += f"; AdvancedDisassembler modülü bulunamadı\n"
                    
            elif format_type == 'enhanced_disasm':
                self.root.after(0, lambda: self.log_message(f"⚡ ENHANCED DISASSEMBLER BAŞLATILIYOR", "INFO"))
                # Enhanced Disassembler - tüm özellikler aktif
                if AdvancedDisassembler:
                    try:
                        disassembler = AdvancedDisassembler(
                            start_address=assembly_start_addr,
                        ...
                        if isinstance(result_code, list):
                            result_code = "\n".join(result_code)
                        self.root.after(0, lambda: self.log_message(f"   ✅ Enhanced Disassembler başarılı", "SUCCESS"))
                    except Exception as e:
                        result_code = f"; Enhanced Disassembler Error: {e}\n"
                        self.root.after(0, lambda: self.log_message(f"   ❌ Enhanced Disassembler hatası: {e}", "ERROR"))
                else:
                    result_code = f"; Enhanced Disassembler - AdvancedDisassembler modülü bulunamadı\n"
            
            # ASM→X TRANSPILER ENGINE - Yeni modüller
            elif format_type.startswith('asm_to_'):
                target_lang = format_type.split('_to_')[1]
                self.root.after(0, lambda: self.log_message(f"🔧 ASM→{target_lang.upper()} TRANSPILER BAŞLATILIYOR", "INFO"))
                
                # Önce assembly disassembly yap
                if AdvancedDisassembler:
                    try:
                        disassembler = AdvancedDisassembler(start_address=start_address, code=code_data)
                        asm_result = disassembler.disassemble()
                        ...
                        # Transpiler'a gönder
                        if target_lang == 'c':
                            result_code = f"// Assembly to C Transpiler v1.0\n"
                            result_code += f"// Original 6502 Assembly -> C Translation\n"
                            result_code += f"// Start Address: 0x{start_address:04X}\n\n"
                            result_code += f"#include <stdio.h>\n"
                            result_code += f"#include <stdint.h>\n\n"
                            result_code += f"// 6502 CPU State\n"
                            result_code += f"typedef struct {{\n"
                            result_code += f"    uint8_t A, X, Y, SP;\n"
                            result_code += f"    uint16_t PC;\n"
                            result_code += f"    uint8_t flags;\n"
                            result_code += f"}} CPU6502;\n\n"
                            result_code += f"void execute_6502_program(CPU6502* cpu, uint8_t* memory) {{\n"
                            
                            # Assembly instruction'ları C koduna çevir (basit örnekler)
                            for line in asm_lines[:20]:  # İlk 20 satır
                                if 'LDA' in line:
                                    result_code += f"    // {line}\n"
                                    result_code += f"    cpu->A = load_accumulator();\n"
                                elif 'STA' in line:
                                    result_code += f"    // {line}\n"
                                    result_code += f"    store_accumulator(cpu->A);\n"
                                elif 'JMP' in line:
                                    result_code += f"    // {line}\n"
                                    result_code += f"    goto jump_target;\n"
                                elif line.strip() and not line.startswith(';'):
                                    result_code += f"    // {line}\n"
                                    result_code += f"    execute_instruction();\n"
                            result_code += f"}}\n"
                            
                        elif target_lang == 'js':
                            result_code = f"// Assembly to JavaScript Transpiler v1.0\n"
                            result_code += f"// Original 6502 Assembly -> JavaScript Translation\n"
                            result_code += f"// Start Address: 0x{start_address:04X}\n\n"
                            result_code += f"class CPU6502 {{\n"
                            result_code += f"    constructor() {{\n"
                            result_code += f"        this.A = 0; this.X = 0; this.Y = 0;\n"
                            result_code += f"        this.PC = 0x{start_address:04X};\n"
                            result_code += f"        this.SP = 0xFF;\n"
                            result_code += f"        this.memory = new Uint8Array(65536);\n"
                            result_code += f"    }}\n\n"
                            result_code += f"    execute() {{\n"
                            
                            # Assembly instruction'ları JS koduna çevir
                            for line in asm_lines[:20]:
                                if 'LDA' in line:
                                    result_code += f"        // {line}\n"
                                    result_code += f"        this.A = this.loadAccumulator();\n"
                                elif 'STA' in line:
                                    result_code += f"        // {line}\n"
                                    result_code += f"        this.storeAccumulator(this.A);\n"
                                elif line.strip() and not line.startswith(';'):
                                    result_code += f"        // {line}\n"
                                    result_code += f"        this.executeInstruction();\n"
                            result_code += f"    }}\n}}\n"
                            
                        elif target_lang == 'python':
                            result_code = f"# Assembly to Python Transpiler v1.0\n"
                            result_code += f"# Original 6502 Assembly -> Python Translation\n"
                            result_code += f"# Start Address: 0x{start_address:04X}\n\n"
                            result_code += f"class CPU6502:\n"
                            result_code += f"    def __init__(self):\n"
                            result_code += f"        self.A = 0\n"
                            result_code += f"        self.X = 0\n"
                            result_code += f"        self.Y = 0\n"
                            result_code += f"        self.PC = 0x{start_address:04X}\n"
                            result_code += f"        self.SP = 0xFF\n"
                            result_code += f"        self.memory = bytearray(65536)\n\n"
                            result_code += f"    def execute(self):\n"
                            
                            # Assembly instruction'ları Python koduna çevir
                            for line in asm_lines[:20]:
                                if 'LDA' in line:
                                    result_code += f"        # {line}\n"
                                    result_code += f"        self.A = self.load_accumulator()\n"
                                elif 'STA' in line:
                                    result_code += f"        # {line}\n"
                                    result_code += f"        self.store_accumulator(self.A)\n"
                                elif line.strip() and not line.startswith(';'):
                                    result_code += f"        # {line}\n"
                                    result_code += f"        self.execute_instruction()\n"
                                    
                        elif target_lang == 'pascal':
                            result_code = f"(* Assembly to Pascal Transpiler v1.0 *)\n"
                            result_code += f"(* Original 6502 Assembly -> Pascal Translation *)\n"
                            result_code += f"(* Start Address: ${start_address:04X} *)\n\n"
                            result_code += f"program C64Emulator;\n\n"
                            result_code += f"type\n"
                            result_code += f"  TCPU6502 = record\n"
                            result_code += f"    A, X, Y, SP: Byte;\n"
                            result_code += f"    PC: Word;\n"
                            result_code += f"    Memory: array[0..65535] of Byte;\n"
                            result_code += f"  end;\n\n"
                            result_code += f"procedure Execute6502(var CPU: TCPU6502);\n"
                            result_code += f"begin\n"
                            
                            # Assembly instruction'ları Pascal koduna çevir
                            for line in asm_lines[:20]:
                                if 'LDA' in line:
                                    result_code += f"  (* {line} *)\n"
                                    result_code += f"  CPU.A := LoadAccumulator(CPU);\n"
                                elif 'STA' in line:
                                    result_code += f"  (* {line} *)\n"
                                    result_code += f"  StoreAccumulator(CPU, CPU.A);\n"
                                elif line.strip() and not line.startswith(';'):
                                    result_code += f"  (* {line} *)\n"
                                    result_code += f"  ExecuteInstruction(CPU);\n"
                            result_code += f"end;\n\n"
                            result_code += f"begin\n"
                            result_code += f"  writeln('6502 Emulator started');\n"
                            result_code += f"end.\n"
                            
                        elif target_lang == 'qbasic':
                            result_code = f"' Assembly to QBasic Transpiler v1.0\n"
                            result_code += f"' Original 6502 Assembly -> QBasic Translation\n"
                            result_code += f"' Start Address: &H{start_address:04X}\n\n"
                            result_code += f"' 6502 CPU Variables\n"
                            result_code += f"DIM RegA AS INTEGER\n"
                            result_code += f"DIM RegX AS INTEGER\n"
                            result_code += f"DIM RegY AS INTEGER\n"
                            result_code += f"DIM PC AS LONG\n"
                            result_code += f"DIM Memory(65535) AS INTEGER\n\n"
                            result_code += f"SUB Execute6502\n"
                            
                            # Assembly instruction'ları QBasic koduna çevir
                            for line in asm_lines[:20]:
                                if 'LDA' in line:
                                    result_code += f"  ' {line}\n"
                                    result_code += f"  RegA = LoadAccumulator\n"
                                elif 'STA' in line:
                                    result_code += f"  ' {line}\n"
                                    result_code += f"  CALL StoreAccumulator(RegA)\n"
                                elif line.strip() and not line.startswith(';'):
                                    result_code += f"  ' {line}\n"
                                    result_code += f"  CALL ExecuteInstruction\n"
                            result_code += f"END SUB\n"
                        
                        elif target_lang == 'rust':
                            result_code = f"// Assembly to Rust Transpiler v1.0\n"
                            result_code += f"// Original 6502 Assembly -> Rust Translation\n"
                            result_code += f"// Start Address: 0x{start_address:04X}\n\n"
                            result_code += f"struct CPU6502 {{\n"
                            result_code += f"    a: u8,\n"
                            result_code += f"    x: u8,\n"
                            result_code += f"    y: u8,\n"
                            result_code += f"    pc: u16,\n"
                            result_code += f"    sp: u8,\n"
                            result_code += f"    memory: [u8; 65536],\n"
                            result_code += f"}}\n\n"
                            result_code += f"impl CPU6502 {{\n"
                            result_code += f"    fn new() -> Self {{\n"
                            result_code += f"        Self {{\n"
                            result_code += f"            a: 0, x: 0, y: 0,\n"
                            result_code += f"            pc: 0x{start_address:04X},\n"
                            result_code += f"            sp: 0xFF,\n"
                            result_code += f"            memory: [0; 65536],\n"
                            result_code += f"        }}\n"
                            result_code += f"    }}\n\n"
                            result_code += f"    fn execute(&mut self) {{\n"
                            ...
                            result_code += f"    }}\n"
                            result_code += f"}}\n\n"
                            result_code += f"fn main() {{\n"
                            result_code += f"    let mut cpu = CPU6502::new();\n"
                            result_code += f"    cpu.execute();\n"
                            result_code += f"}}\n"
                        
                        elif target_lang == 'c2':
                            result_code = f"// Assembly to Modern C (C2) Transpiler v1.0\n"
                            result_code += f"// Original 6502 Assembly -> Modern C Translation\n"
                            result_code += f"// Start Address: 0x{start_address:04X}\n\n"
                            result_code += f"#include <stdio.h>\n"
                            result_code += f"#include <stdint.h>\n"
                            result_code += f"#include <stdbool.h>\n\n"
                            result_code += f"typedef struct {{\n"
                            result_code += f"    uint8_t a, x, y, sp;\n"
                            result_code += f"    uint16_t pc;\n"
                            result_code += f"    bool carry, zero, interrupt, decimal, overflow, negative;\n"
                            result_code += f"    uint8_t memory[65536];\n"
                            result_code += f"}} cpu_6502_t;\n\n"
                            result_code += f"static inline void execute_6502_modern(cpu_6502_t* const cpu) {{\n"
                            
                            for line in asm_lines[:20]:
                                if 'LDA' in line:
                                    result_code += f"    // {line}\n"
                                    result_code += f"    cpu->a = load_accumulator_modern(cpu);\n"
                                elif 'STA' in line:
                                    result_code += f"    // {line}\n"
                                    result_code += f"    store_accumulator_modern(cpu, cpu->a);\n"
                                elif line.strip() and not line.startswith(';'):
                                    result_code += f"    // {line}\n"
                                    result_code += f"    execute_instruction_modern(cpu);\n"
                            result_code += f"}}\n"
                        
                        elif target_lang == 'pdsx':
                            result_code = f"# Assembly to PDSX BASIC Transpiler v1.0\n"
                            result_code += f"# Original 6502 Assembly -> PDSX BASIC Translation\n"
                            result_code += f"# Start Address: ${start_address:04X}\n\n"
                            result_code += f"DEF CPU6502()\n"
                            result_code += f"    GLOBAL A%, X%, Y%, PC%, SP%\n"
                            result_code += f"    GLOBAL Memory%(65535)\n"
                            result_code += f"    A% = 0: X% = 0: Y% = 0\n"
                            result_code += f"    PC% = &H{start_address:04X}\n"
                            result_code += f"    SP% = &HFF\n"
                            result_code += f"END DEF\n\n"
                            result_code += f"DEF Execute6502()\n"
                            
                            for line in asm_lines[:20]:
                                if 'LDA' in line:
                                    result_code += f"    REM {line}\n"
                                    result_code += f"    A% = LoadAccumulator%()\n"
                                elif 'STA' in line:
                                    result_code += f"    REM {line}\n"
                                    result_code += f"    StoreAccumulator%(A%)\n"
                                elif line.strip() and not line.startswith(';'):
                                    result_code += f"    REM {line}\n"
                                    result_code += f"    ExecuteInstruction%()\n"
                            result_code += f"END DEF\n"
                            ...
                            result_code += f"// Original 6502 Assembly -> C++ Translation\n"
                            result_code += f"// Start Address: 0x{start_address:04X}\n\n"
                            result_code += f"#include <iostream>\n"
                            result_code += f"#include <cstdint>\n"
                            result_code += f"#include <array>\n\n"
                            result_code += f"class CPU6502 {{\n"
                            result_code += f"private:\n"
                            result_code += f"    uint8_t a, x, y, sp;\n"
                            result_code += f"    uint16_t pc;\n"
                            result_code += f"    std::array<uint8_t, 65536> memory;\n\n"
                            result_code += f"public:\n"
                            result_code += f"    CPU6502() : a(0), x(0), y(0), sp(0xFF), pc(0x{start_address:04X}) {{\n"
                            result_code += f"        memory.fill(0);\n"
                            result_code += f"    }}\n\n"
                            result_code += f"    void execute() {{\n"
                            ...
                            result_code += f"    }}\n"
                            result_code += f"}};\n\n"
                            result_code += f"int main() {{\n"
                            result_code += f"    CPU6502 cpu;\n"
                            result_code += f"    cpu.execute();\n"
                            result_code += f"    return 0;\n"
                            result_code += f"}}\n"
                        
                        else:
                            result_code = f"; ASM→{target_lang.upper()} Transpiler v1.0\n"
                            result_code += f"; Target language '{target_lang}' not yet implemented\n"
                            result_code += f"; Original 6502 Assembly:\n\n"
                            for line in asm_lines[:50]:
                                result_code += f"{line}\n"
                        
                        self.root.after(0, lambda: self.log_message(f"   ✅ ASM→{target_lang.upper()} Transpiler başarılı", "SUCCESS"))
                        
                    except Exception as transpiler_error:
                        error_msg = f"ASM→{target_lang.upper()} Transpiler exception: {transpiler_error}"
                        self.root.after(0, lambda: self.log_message(f"   ❌ {error_msg}", "ERROR"))
                        result_code = f"// ASM→{target_lang.upper()} TRANSPILER ERROR: {error_msg}\n"
                        result_code += f"// Falling back to basic disassembly\n\n"
                        if asm_result:
                            result_code += f"/*\nOriginal Assembly:\n{asm_result}\n*/\n"
                else:
                    result_code = f"; ASM→{target_lang.upper()} TRANSPILER MODULE ERROR\n"
                    result_code += f"; AdvancedDisassembler modülü bulunamadı\n"
                    self.root.after(0, lambda: self.log_message(f"   ❌ AdvancedDisassembler modülü bulunamadı", "ERROR"))
            
            # Sanal yazım modları (Virtual Writing Modes)
            elif format_type == 'c':
                # C transpilation
                self.root.after(0, lambda: self.log_message(f"🔧 C TRANSPİLATİON BAŞLATILIYOR", "INFO"))
                if AdvancedDisassembler:
                    try:
                        disassembler = AdvancedDisassembler(start_address=start_address, code=code_data)
                        asm_code = disassembler.disassemble()
                        ...
                        result_code += f"    printf(\"6502 Program Execution\\n\");\n"
                        result_code += f"    printf(\"Start Address: 0x{start_address:04X}\\n\");\n"
                        result_code += f"    \n"
                        ...
                        for line in asm_code.split('\n')[:20]:
                            if line.strip() and not line.startswith(';'):
                                result_code += f"    // {line}\n"
                        result_code += f"}}\n"
                        self.root.after(0, lambda: self.log_message(f"   ✅ C Transpilation başarılı", "SUCCESS"))
                    except Exception as e:
                        result_code = f"// C Transpilation Error: {e}\n"
                        self.root.after(0, lambda: self.log_message(f"   ❌ C Transpilation hatası: {e}", "ERROR"))
                else:
                    result_code = f"// AdvancedDisassembler modülü bulunamadı\n"
                    result_code += f"// Start Address: ${start_address:04X}\n"
                ...
                self.root.after(0, lambda: self.log_message(f"📝 QBASIC TRANSPİLATİON BAŞLATILIYOR", "INFO"))
                ...
                if AdvancedDisassembler:
                    try:
                        ...
                        for line in asm_code.split('\n')[:20]:  # İlk 20 satır
                            if line.strip():
                                result_code += f"' {line}\n"
                        self.root.after(0, lambda: self.log_message(f"   ✅ QBasic Transpilation başarılı", "SUCCESS"))
                    except Exception as e:
                        result_code += f"' QBasic Transpilation Error: {e}\n"
                        self.root.after(0, lambda: self.log_message(f"   ❌ QBasic Transpilation hatası: {e}", "ERROR"))
                
            elif format_type == 'pdsx':
                # PDSX format
                self.root.after(0, lambda: self.log_message(f"🔧 PDSX FORMAT BAŞLATILIYOR", "INFO"))
                ...
                if AdvancedDisassembler:
                    try:
                        ...
                        for line in asm_lines:
                            if line.strip() and not line.startswith(';'):
                                result_code += f"{line}\n"
                        self.root.after(0, lambda: self.log_message(f"   ✅ PDSX Format başarılı", "SUCCESS"))
                    except Exception as e:
                        result_code += f"; PDSX Format Error: {e}\n"
                        self.root.after(0, lambda: self.log_message(f"   ❌ PDSX Format hatası: {e}", "ERROR"))
                else:
                    result_code += f"; AdvancedDisassembler modülü bulunamadı\n"
                        
            elif format_type == 'pseudo':
                # Pseudo code - Enhanced version
                self.root.after(0, lambda: self.log_message(f"💭 PSEUDO CODE BAŞLATILIYOR", "INFO"))
                ...
                # Basit program akış analizi
                if len(code_data) > 10:
                    result_code += f"        LOAD_PROGRAM_DATA({len(code_data)} bytes)\n"
                    if code_data[0] == 0xA9:  # LDA immediate
                        result_code += f"        LOAD_ACCUMULATOR_WITH_VALUE(${code_data[1]:02X})\n"
                    if 0x4C in code_data:  # JMP absolute
                        result_code += f"        JUMP_TO_SUBROUTINE()\n"
                    if 0x20 in code_data:  # JSR absolute
                        result_code += f"        CALL_SUBROUTINE()\n"
                    result_code += f"        EXECUTE_6502_INSTRUCTIONS()\n"
                    result_code += f"        HANDLE_INTERRUPTS()\n"
                else:
                    result_code += f"        EXECUTE_MINIMAL_PROGRAM()\n"
                    
                result_code += f"    }}\n"
                result_code += f"}}\n"
                self.root.after(0, lambda: self.log_message(f"   ✅ Pseudo Code başarılı", "SUCCESS"))
                result_code += f"// Code Size: {len(code_data)} bytes\n\n"
                result_code += f"PROGRAM C64_APPLICATION {{\n"
                result_code += f"    ENTRY_POINT: ${start_address:04X}\n"
                result_code += f"    \n"
            ...
            if result_code:
                ...
                if isinstance(result_code, list):
                    ...
                    result_code = str(result_code)
            # Sonuç kodunu SADECE PANELLerde göster - RESULT WINDOW OLUŞTURMA!
            if result_code and len(result_code.strip()) > 0:
                # Disassembly formatları - DisassemblyPanel'e gönder
                if format_type in ['assembly', 'advanced_disasm', 'improved_disasm', 'hybrid_disasm', 'py65_pro', 'enhanced_disasm', 'multi_disasm',
                                 'basic', 'petcat', 'dlist', 'pdsx', 'c', 'qbasic', 'pseudo'] or format_type.startswith('assembly_') or format_type.startswith('asm_to_'):
                    self.root.after(0, lambda: self.disassembly_panel.update_code(result_code, format_type))
                    self.root.after(0, lambda: self.log_message(f"✅ DisassemblyPanel güncellendi: {format_type}", "INFO"))
                
                # Decompiler formatları - DecompilerPanel'e gönder  
                elif format_type.startswith('dec_') or format_type.startswith('transpile_') or format_type == 'enhanced_basic' or format_type == 'decompiler':
                    self.root.after(0, lambda: self.decompiler_panel.update_code(result_code, format_type))
                    self.root.after(0, lambda: self.log_message(f"✅ DecompilerPanel güncellendi: {format_type}", "INFO"))
                
                # RESULT WINDOW OLUŞTURULMAYACAK - SADECE PANELLER GÜNCELLENCEK
                self.root.after(0, lambda: self.log_message(f"✅ Panel güncellendi, result window oluşturulmadı: {format_type}", "INFO"))
            else:
                # Boş sonuç durumu
                error_msg = f"❌ {format_type} dönüştürme boş sonuç döndü"
                self.root.after(0, lambda: self.log_message(error_msg, "ERROR"))
                self.root.after(0, lambda: self.update_status(f"{format_type} dönüştürme boş sonuç"))
                return
            ...
            # Sonuç başarı kontrolü - hata mesajı içerip içermediğini kontrol et
            is_success = not any(error_indicator in result_code.lower() for error_indicator in [
                'error:', 'exception:', 'failed:', 'hatası:', 'hata:',
                'bulunamadı', 'not found', 'no attribute', 'cannot access'
            ])
            
            if is_success:
                self.root.after(0, lambda: self.log_message(f"✅ {format_type} dönüştürme başarılı: {len(result_code)} karakter", "SUCCESS"))
            else:
                self.root.after(0, lambda: self.log_message(f"❌ {format_type} dönüştürme başarısız - hata mesajı üretildi: {len(result_code)} karakter", "ERROR"))
    ...
    def batch_save_all_outputs(self):
        """Tüm çıktı tablarını kaydet - Disk Directory düğmesi için"""
        if not hasattr(self, 'disassembly_panel') or not self.disassembly_panel:
            messagebox.showwarning("Uyarı", "Disassembly panel bulunamadı!")
            return
        
        if not hasattr(self.disassembly_panel, 'result_tabs') or not self.disassembly_panel.result_tabs:
            messagebox.showwarning("Uyarı", "Kaydetmek için sonuç tabları bulunamadı!")
            return
        
        # Klasör seç
        output_dir = filedialog.askdirectory(title="Tüm çıktıları kaydetmek için klasör seçin")
        if not output_dir:
            return
        
        try:
            saved_count = 0
            base_filename = getattr(self, 'current_file_name', 'output')
            if '.' in base_filename:
                base_filename = base_filename.split('.')[0]
            
            # Her tab için kaydet
            for tab_name, tab_info in self.disassembly_panel.result_tabs.items():
                ...
                if text_widget:
                    try:
                        ...
                        if content:
                            ...
                            ext_map = {
                                'assembly': '.asm', 'enhanced': '.asm', 'memmap': '.asm',
                                'dasm': '.asm', 'tass': '.asm', 'kickass': '.asm', 'acme': '.asm',
                                'ca65': '.s', 'css64': '.asm', 'supermon': '.asm',
                                'c': '.c', 'cpp': '.cpp', 'qbasic': '.bas', 'pdsx': '.pdsx',
                                'pseudo': '.txt', 'commodore_basic': '.bas', 'enhanced_basic': '.bas'
                            }
                            
                            ext = ext_map.get(tab_name.lower(), '.txt')
                            filename = os.path.join(output_dir, f"{base_filename}_{tab_name}{ext}")
                            
                            with open(filename, 'w', encoding='utf-8') as f:
                                f.write(content)
                            
                            saved_count += 1
                            self.log_message(f"✅ Kaydedildi: {filename}", "INFO")
                    
                    except Exception as e:
                        self.log_message(f"❌ {tab_name} tab kaydetme hatası: {e}", "ERROR")
            
            if saved_count > 0:
                messagebox.showinfo("Başarılı", f"✅ {saved_count} adet dosya başarıyla kaydedildi:\n{output_dir}")
                self.update_status(f"✅ {saved_count} dosya kaydedildi")
            else:
                messagebox.showwarning("Uyarı", "Kaydedilecek içerik bulunamadı!")
        
        except Exception as e:
            error_msg = f"Toplu kaydetme hatası: {e}"
            self.log_message(error_msg, "ERROR")
            messagebox.showerror("Hata", error_msg)
    
    def save_as_current_code(self):
        """Farklı kaydet - alias for save_as_code"""
        self.save_as_code()
...
"""
    ...
    def _format_enhanced_hybrid_report(self, analysis_result):
        """Enhanced hibrit analiz raporu oluştur - İyileştirilmiş hesaplama"""
        if not analysis_result:
            return "❌ Analiz başarısız"
        
        report = []
        report.append("=" * 70)
        report.append("🔍 ENHANCED HİBRİT PROGRAM ANALİZİ v2.0")
        report.append("=" * 70)
        
        # Genel bilgiler
        report.append(f"\n📊 GENEL BİLGİLER:")
        report.append(f"   Yükleme Adresi: ${analysis_result['load_address']:04X} ({analysis_result['load_address']})")
        ...
        if analysis_result.get('basic_section'):
            ...
            report.append(f"\n📝 BASIC SECTION (İyileştirilmiş Tespit):")
            report.append(f"   📍 Başlangıç Adresi: ${basic['start_address']:04X} ({basic['start_address']})")
            report.append(f"   📍 Bitiş Adresi: ${basic['end_address']:04X} ({basic['end_address']})")
            report.append(f"   📏 Boyut: {basic['size']} byte")
            report.append(f"   📏 Adres Aralığı: ${basic['start_address']:04X}-${basic['end_address']:04X}")
            
            # BASIC data preview
            if basic.get('data') and len(basic['data']) > 0:
                preview_size = min(20, len(basic['data']))
                hex_preview = ' '.join(f"{b:02X}" for b in basic['data'][:preview_size])
                report.append(f"   🔍 İlk {preview_size} byte: {hex_preview}")
            
        # Assembly section analizi  
        if analysis_result.get('assembly_section'):
            asm = analysis_result['assembly_section']
            report.append(f"\n⚙️ ASSEMBLY SECTION (İyileştirilmiş Tespit):")
            report.append(f"   📍 Başlangıç Adresi: ${asm['start_address']:04X} ({asm['start_address']})")
            report.append(f"   📍 Bitiş Adresi: ${asm['end_address']:04X} ({asm['end_address']})")
            report.append(f"   📏 Boyut: {asm['size']} byte")
            report.append(f"   📏 Adres Aralığı: ${asm['start_address']:04X}-${asm['end_address']:04X}")
            
            # Assembly data preview
            if asm.get('data') and len(asm['data']) > 0:
                preview_size = min(20, len(asm['data']))
                hex_preview = ' '.join(f"{b:02X}" for b in asm['data'][:preview_size])
                report.append(f"   🔍 İlk {preview_size} byte: {hex_preview}")
        
        # Bellek haritası
        report.append(f"\n🗺️ BELLEK HARİTASI:")
        total_addr_start = analysis_result['load_address']
        total_addr_end = analysis_result['load_address'] + analysis_result['total_size']
        report.append(f"   📍 Toplam Adres Aralığı: ${total_addr_start:04X}-${total_addr_end:04X}")
        
        if analysis_result.get('basic_section') and analysis_result.get('assembly_section'):
            basic = analysis_result['basic_section']
            asm = analysis_result['assembly_section']
            overlap = basic['end_address'] - asm['start_address']
            if overlap > 0:
                report.append(f"   ⚠️  UYARI: {overlap} byte çakışma tespit edildi!")
            else:
                gap = asm['start_address'] - basic['end_address']
                if gap > 0:
                    report.append(f"   📏 BASIC-Assembly arası boşluk: {gap} byte")
                else:
                    report.append(f"   ✅ BASIC ve Assembly bölümleri düzgün sıralanmış")
        
        # Öneriler
        report.append(f"\n💡 ÖNERİLER:")
        if analysis_result['analysis'] == 'hybrid_program':
            report.append("   ✅ Bu hibrit bir program (BASIC + Assembly)")
            report.append("   🔧 BASIC kısmını Enhanced BASIC Decompiler ile analiz edin")
            report.append("   🔧 Assembly kısmını Advanced Disassembler ile disassemble edin")
            if analysis_result.get('sys_address'):
                report.append(f"   🎯 SYS {analysis_result['sys_address']} ile Assembly kısmı çalıştırılıyor")
        elif analysis_result['analysis'] == 'pure_assembly':
            report.append("   ✅ Bu saf assembly programı")
            report.append("   🔧 Doğrudan Advanced Disassembler ile disassemble edin")
        
        # Teknik detaylar
        report.append(f"\n🔬 TEKNİK DETAYLAR:")
        report.append(f"   🧮 Analiz Algoritması: Enhanced BASIC Line Structure Detection v2.0")
        report.append(f"   🎯 BASIC Terminator Detection: İyileştirilmiş pointer validation")
        report.append(f"   🔍 SYS Address Detection: Multi-pattern regex analysis")
