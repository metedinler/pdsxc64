...
class UniversalC64Engine:
    ...
    def __init__(self, parent_widget=None, event_manager=None):
        ...
        self.command_handlers = {
            'C64_INIT': self._handle_c64_init_command,
            'C64_CLEAR': self._handle_c64_clear_command, 
            'C64_PIXEL': self._handle_c64_pixel_command,
            'C64_CHAR': self._handle_c64_char_command,
            ...
            'POKE': self._handle_poke_command,
            'PEEK': self._handle_peek_command
        }
    ...
    # PDSX uyumlu wrapper metodları
    def set_pixel(self, x: int, y: int, color: int):
        """PDSX uyumlu pixel set metodu"""
        return self.display.set_pixel(x, y, color)
    
    def draw_char(self, x: int, y: int, char_code: int, color: int = 1):
        """PDSX uyumlu character draw metodu"""
        return self.display.draw_char(x, y, char_code, color)
    
    def create_sprite(self, sprite_id: int, width: int, height: int, color: int):
        """PDSX uyumlu sprite create metodu"""
        return self.sprite_manager.create_sprite(sprite_id, width, height, color)
        
    def set_screen_background(self, color: int):
        """PDSX uyumlu background color metodu"""
        self.memory.background_color = color
        self.display.clear_screen(color)
        
    def set_screen_border(self, color: int):
        """PDSX uyumlu border color metodu"""
        self.memory.border_color = color
        # Border color UI'da güncellenmeli
    ...
    def execute_command(self, command: str) -> bool:
        ...
        try:
            ...
            if cmd_type in self.command_handlers:
                ...
                # Silent fallback for unknown commands
                return False
    ...
    def _handle_c64_init_command(self, command: str) -> bool:
        """C64_INIT komutunu işle"""
        try:
            self.initialize()
            return True
        except Exception as e:
    ...
    def _handle_c64_clear_command(self, command: str) -> bool:
        """C64_CLEAR komutunu işle"""
        try:
            self.display.clear_screen(self.memory.background_color)
            return True
        except Exception as e:
    ...
    def _handle_c64_pixel_command(self, command: str) -> bool:
        """C64_PIXEL komutunu işle"""
        try:
            parts = command.split()
            if len(parts) >= 4:
    ...
    def _handle_c64_char_command(self, command: str) -> bool:
        """C64_CHAR komutunu işle"""
        try:
            parts = command.split()
            if len(parts) >= 5:
                x, y, char_code, color = int(parts[1]), int(parts[2]), int(parts[3]), int(parts[4])
                self.display.set_character(x, y, char_code, color)
            return True
        except Exception as e:
            print(f"C64_CHAR hatası: {e}")
            return False
    
    def _handle_poke_command(self, command: str) -> bool:
        """POKE komutunu işle"""
        try:
            parts = command.split()
            if len(parts) >= 3:
                address, value = int(parts[1]), int(parts[2])
    ...
    def _handle_peek_command(self, command: str) -> bool:
        """PEEK komutunu işle"""
        try:
            parts = command.split()
            if len(parts) >= 2:
                ...
                return value  # PEEK değer döndürür
            return 0
        except Exception as e:
...
# PDSX Integration - Engine instance getter
_engine_instance = None

def get_c64_gui_engine():
    """PDSX için C64 GUI Engine instance'ı döndür (singleton)"""
    global _engine_instance
    if _engine_instance is None:
        try:
            import tkinter as tk
            root = tk.Tk()
            root.withdraw()  # Gizle, sadece engine için
            _engine_instance = UniversalC64Engine(root)
        except Exception as e:
            print(f"C64 GUI Engine oluşturma hatası: {e}")
            _engine_instance = None
