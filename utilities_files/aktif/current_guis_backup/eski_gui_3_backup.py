...
class D64ConverterApp:
    ...
    def analyze_illegal_opcodes(self):
        ...
        try:
            # Illegal analyzer kontrol
            if not ILLEGAL_ANALYZER_AVAILABLE or not self.illegal_analyzer:
                messagebox.showwarning("Uyarı", "Illegal Opcode Analyzer mevcut değil!")
                return
            ...
            print(f"Illegal opcode analizi: {selected_entry}")
            ...
            # PRG dosyasından start_address ve code'u çıkar
            if len(prg_data) < 2:
                self.status_var.set("Geçersiz PRG verisi")
