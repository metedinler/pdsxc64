...
def print_banner():
    ...
    banner_lines = [
        ...
        f"{Colors.HEADER}║                                                                            ║{Colors.ENDC}",
        f"{Colors.HEADER}║  {Colors.BOLD}🚀 D64 CONVERTER v5.0 - SUPER UNIFIED EDITION 🚀{Colors.ENDC}{Colors.HEADER}                 ║{Colors.ENDC}",
        f"{Colors.HEADER}║                                                                            ║{Colors.ENDC}",
        f"{Colors.HEADER}║  {Colors.OKBLUE}Advanced Commodore 64 Decompiler Suite{Colors.ENDC}{Colors.HEADER}                             ║{Colors.ENDC}",
        f"{Colors.HEADER}║  {Colors.OKCYAN}✨ Assembly Formatters (8 formats) + Decompiler Languages (5 formats){Colors.ENDC}{Colors.HEADER}║{Colors.ENDC}",
        f"{Colors.HEADER}║  {Colors.OKGREEN}🔧 Enhanced Memory Manager • 🧠 AI Pattern Recognition{Colors.ENDC}{Colors.HEADER}              ║{Colors.ENDC}",
        f"{Colors.HEADER}║  {Colors.WARNING}📊 94% Success Rate • 🎯 Production Ready{Colors.ENDC}{Colors.HEADER}                          ║{Colors.ENDC}",
        f"{Colors.HEADER}║                                                                            ║{Colors.ENDC}",
        f"{Colors.HEADER}╚════════════════════════════════════════════════════════════════════════════╝{Colors.ENDC}",
        "",
        f"{Colors.BOLD}{Colors.OKCYAN}🎯 SUPER UNIFIED SYSTEM - MAIN + ULTIMATE COMBINED{Colors.ENDC}",
...
def launch_modern_gui(theme='light'):
    """Launch Modern GUI v5.0 with theme support - Enhanced error reporting"""
    logger = logging.getLogger(__name__)
    ...
    try:
        ...
        try:
            logger.info("📋 Attempting to import gui_demo...")
            from gui_demo import launch_gui
            logger.info("✅ gui_demo imported successfully")
            logger.info("🎨 Launching gui_demo interface...")
            launch_gui()
            logger.info("✅ gui_demo completed successfully")
        except ImportError as e:
            logger.warning(f"⚠️ gui_demo import failed: {e}")
            logger.info("📋 Fallback: Trying gui_manager...")
            try:
                from gui_manager import D64ConverterGUI
                import tkinter as tk
                logger.info("✅ gui_manager imported successfully")
                logger.info("🎨 Creating tkinter root window...")
                root = tk.Tk()
                logger.info("🎨 Initializing D64ConverterGUI...")
                app = D64ConverterGUI(root)
                logger.info("✅ D64ConverterGUI initialized successfully")
                logger.info("🚀 Starting GUI main loop...")
                root.mainloop()
                logger.info("✅ GUI main loop completed")
            except Exception as gui_error:
                logger.error(f"❌ gui_manager initialization failed: {gui_error}")
                import traceback
            ...
            logger.error(f"❌ gui_demo execution failed: {demo_error}")
            import traceback
            logger.error(f"Full traceback:\n{traceback.format_exc()}")
            print(f"\n❌ GUI DEMO ERROR:")
        ...
        error_msg = f"❌ Modern GUI import error: {e}"
        logger.error(error_msg)
        print(f"\n{error_msg}")
        print("🔧 Possible solutions:")
        print("   • Check if gui_manager.py exists")
        print("   • Check if gui_demo.py exists") 
        print("   • Verify all dependencies are installed")
        print("   • Try running: python -c 'import tkinter; print(\"Tkinter OK\")'")
        return False
    except Exception as e:
        error_msg = f"❌ GUI launch error: {e}"
        logger.error(error_msg)
        import traceback
        ...
        print("   1. Check if all GUI files exist")
        print("   2. Verify Python tkinter installation")
        print("   3. Check virtual environment setup")
        print("   4. Review log files in logs/ directory")
