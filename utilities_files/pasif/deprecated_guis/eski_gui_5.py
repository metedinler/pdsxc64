...
class D64ConverterApp:
    def __init__(self, root):
        ...
        self.output_format = tk.StringVar(value='asm')  # 'asm', 'c', 'qbasic', 'pseudo'
        self.entries = []
    ...
    def setup_gui(self):
        ...
        # Çeviri formatı seçimi
        format_frame = ttk.Frame(options_frame)
        format_frame.grid(row=1, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(10, 0))
        
        ttk.Label(format_frame, text="Çıktı Formatı:").grid(row=0, column=0, sticky=tk.W)
        ...
        format_options = [
            ...
            ('QBasic', 'qbasic'),
            ('PDSX', 'pdsx'),
            ('Commodore BASIC V2', 'commodorebasicv2'),
            ('Pseudo Code', 'pseudo')
        ]
        
        # Format seçeneklerini 2 satıra böl
        for i, (text, value) in enumerate(format_options):
            row = i // 3  # 3'er tane her satırda
            col = i % 3 + 1
            ttk.Radiobutton(format_frame, text=text, variable=self.output_format, 
                           value=value).grid(row=row, column=col, sticky=tk.W, padx=(10, 0), pady=2)
    ...
    def _update_all_outputs(self, assembly_code, disassembler):
        ...
        try:
            ...
            print(f"C kodu uzunluğu: {len(c_code)}")  # Debug
            ...
            print(f"QBasic kodu uzunluğu: {len(qbasic_code)}")  # Debug
            ...
            print(f"PDSX kodu uzunluğu: {len(pdsx_code)}")  # Debug
            ...
            print(f"Pseudo kodu uzunluğu: {len(pseudo_code)}")  # Debug
            ...
            print(f"BASIC kodu uzunluğu: {len(basic_code)}")  # Debug
