#!/usr/bin/env python3
"""
D64 Converter - GUI Manager v5.0 - GUI DEBUG MODE
Modern Tkinter tabanlı grafik arayüz - X1 GUI Integration + GUI Debug System
...
ADIM 5: GUI Integration + X1 Features + GUI Debug
...
Version: 5.0 (X1 Integration + GUI Debug)
"""
...
def debug_button(parent, debug_name="Button", text="", **kwargs):
    """Debug-enabled Button wrapper - IMPROVED WITH REGISTRY"""
    try:
        original_text = text
...
def debug_notebook(parent, debug_name="Notebook", **kwargs):
    """Debug-enabled Notebook wrapper"""
...
def debug_filedialog_open(debug_name="File Dialog", **kwargs):
    """Debug-enabled File Dialog Open wrapper"""
...
class DiskDirectoryPanel(tk.Frame):
    ...
    def setup_ui(self):
        ...
        # Header
        header_frame = debug_frame(self, debug_name="Directory Header", bg=ModernStyle.BG_SECONDARY)
        header_frame.pack(fill=tk.X, padx=5, pady=2)
        
        debug_label(header_frame, debug_name="Directory Title", text="📁 Disk Directory", bg=ModernStyle.BG_SECONDARY,
                fg=ModernStyle.FG_PRIMARY, font=("Arial", 12, "bold")).pack(side=tk.LEFT, padx=5)
        
        # File controls
        controls_frame = debug_frame(self, debug_name="File Controls", bg=ModernStyle.BG_SECONDARY)
        controls_frame.pack(fill=tk.X, padx=5, pady=2)
        
        debug_button(controls_frame, debug_name="Select File", text="📂 Seç", command=self.select_file).pack(side=tk.LEFT, padx=2)
        debug_button(controls_frame, debug_name="Find Files", text="🔍 Dosya Bul", command=self.find_files).pack(side=tk.LEFT, padx=2)
        debug_button(controls_frame, debug_name="Show Processed", text="📋 İşlenenler", command=self.show_processed_files).pack(side=tk.LEFT, padx=2)
        
        # Modern save/export controls  
        debug_button(controls_frame, debug_name="Save Current", text="💾 Kaydet", command=self.parent_gui.save_current_code if self.parent_gui else self.save_current_code).pack(side=tk.LEFT, padx=2)
        debug_button(controls_frame, debug_name="Save As", text="📁 Farklı Kaydet", command=self.parent_gui.save_as_code if self.parent_gui else self.save_as_code).pack(side=tk.LEFT, padx=2)
        debug_button(controls_frame, debug_name="Export All", text="📦 Tümünü Aktar", command=self.parent_gui.export_all_formats if self.parent_gui else self.export_all_formats).pack(side=tk.LEFT, padx=2)
        
        # Directory tree
        tree_frame = debug_frame(self, debug_name="Directory Tree")
        tree_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Treeview columns - X1 GUI formatı
        columns = ("filename", "filetype", "start_addr", "end_addr", "program_type", "track", "sector", "size")
        self.tree = debug_treeview(tree_frame, debug_name="Disk Directory Tree", columns=columns, show="headings", height=15)
        ...
        # Scrollbar
        scrollbar = debug_scrollbar(tree_frame, debug_name="Disk Directory Scrollbar", orient=tk.VERTICAL, command=self.tree.yview)
        ...
        # Analysis buttons
        analysis_frame = debug_frame(self, debug_name="Analysis Controls", bg=ModernStyle.BG_SECONDARY)
        analysis_frame.pack(fill=tk.X, padx=5, pady=2)
        
        debug_button(analysis_frame, debug_name="Illegal Analysis", text="🚫 Illegal Analiz", command=lambda: self.parent_gui.analyze_illegal_opcodes_current() if self.parent_gui else None).pack(side=tk.LEFT, padx=2)
        debug_button(analysis_frame, debug_name="Sprite Analysis", text="🎮 Sprite Analiz", command=lambda: self.parent_gui.analyze_sprites_current() if self.parent_gui else None).pack(side=tk.LEFT, padx=2)
        debug_button(analysis_frame, debug_name="SID Analysis", text="🎵 SID Analiz", command=lambda: self.parent_gui.analyze_sid_current() if self.parent_gui else None).pack(side=tk.LEFT, padx=2)
        debug_button(analysis_frame, debug_name="Charset Analysis", text="🔤 Charset Analiz", command=lambda: self.parent_gui.analyze_charset_current() if self.parent_gui else None).pack(side=tk.LEFT, padx=2)
        
        # Hybrid Analysis row
        hybrid_frame = debug_frame(self, debug_name="Hybrid Analysis", bg=ModernStyle.BG_SECONDARY)
        hybrid_frame.pack(fill=tk.X, padx=5, pady=2)
        
        debug_button(hybrid_frame, debug_name="Hybrid Analysis", text="🔍 Hibrit Analiz", command=lambda: self.parent_gui.analyze_hybrid_program_current() if self.parent_gui else None).pack(side=tk.LEFT, padx=2)
        debug_button(hybrid_frame, debug_name="Extract Assembly", text="⚙️ Assembly Ayır", command=lambda: self.parent_gui.extract_assembly_current() if self.parent_gui else None).pack(side=tk.LEFT, padx=2)
        debug_button(hybrid_frame, debug_name="Extract BASIC", text="📝 BASIC Ayır", command=lambda: self.parent_gui.extract_basic_current() if self.parent_gui else None).pack(side=tk.LEFT, padx=2)
    ...
    def show_found_files(self, files):
        ...
        file_window = debug_toplevel(self.parent_gui.root if self.parent_gui else None, 
                                   title="Bulunan C64 Dosyaları")
        file_window.geometry("600x400")
        file_window.grab_set()
        
        debug_label(file_window, text=f"Bulunan C64 dosyaları ({len(files)} adet):").pack(pady=10)
        
        # Listbox
        listbox_frame = debug_frame(file_window, debug_name="ListBox Container")
        listbox_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        
        scrollbar = debug_scrollbar(listbox_frame, debug_name="File List Scrollbar")
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        listbox = debug_listbox(listbox_frame, debug_name="File List", yscrollcommand=scrollbar.set)
        ...
        # Butonlar
        button_frame = debug_frame(file_window, debug_name="File Dialog Buttons")
        ...
        def select_from_list():
            ...
            if selection:
                ...
                safe_messagebox("warning", "Seçim Yok", "Lütfen bir dosya seçin")
        
        debug_button(button_frame, text="Seç", command=select_from_list).pack(side=tk.LEFT, padx=5)
        debug_button(button_frame, text="İptal", command=file_window.destroy).pack(side=tk.LEFT, padx=5)
...
class DisassemblyPanel(tk.Frame):
    ...
    def setup_ui(self):
        ...
        debug_label(header_frame, debug_name="Disassembly Header", text="⚙️ Disassembly", bg=ModernStyle.BG_SECONDARY,
                fg=ModernStyle.FG_PRIMARY, font=("Arial", 12, "bold")).pack(side=tk.LEFT, padx=5)
        ...
        # BASIC Detokenizers grubu - yukarıya taşındı
        ...
        basic_group = debug_frame(group2, debug_name="Basic Button Group", bg=ModernStyle.BG_SECONDARY)
        basic_group.pack(side=tk.TOP)
        
        debug_button(basic_group, debug_name="BASIC Parser", text="BASIC Parser", command=lambda: self.convert_format('basic')).grid(row=0, column=0, padx=1)
        debug_button(basic_group, debug_name="Petcat", text="Petcat", command=lambda: self.convert_format('petcat')).grid(row=0, column=1, padx=1)
        debug_button(basic_group, debug_name="C64List", text="C64List", command=lambda: self.convert_format('c64list')).grid(row=0, column=2, padx=1)
        
        # Disassembly Modülleri grubu
        group1 = debug_frame(format_frame, debug_name="Disassembly Group", bg=ModernStyle.BG_SECONDARY)
        group1.pack(side=tk.LEFT, padx=5, pady=2)
        
        debug_label(group1, debug_name="Disassembly Modules Label", text="Disassembly Modules:", bg=ModernStyle.BG_SECONDARY,
                fg=ModernStyle.FG_ACCENT, font=("Arial", 9, "bold")).pack(side=tk.TOP, pady=1)
        
        # Assembly format dropdown ekle
        asm_frame = debug_frame(group1, debug_name="ASM Frame")
        asm_frame.pack(side=tk.TOP, pady=1)
        
        debug_label(asm_frame, debug_name="ASM Label", text="ASM:", font=("Arial", 8)).pack(side=tk.LEFT)
        self.asm_format_var = tk.StringVar(value="Native")
        asm_formats = ["Native", "ACME", "CC65", "DASM", "KickAss", "TASM", "64TASS", "CA65"]
        self.asm_format_combo = debug_combobox(asm_frame, debug_name="ASM Format Combo", textvariable=self.asm_format_var, 
                                           values=asm_formats, width=8, state="readonly")
        self.asm_format_combo.pack(side=tk.LEFT, padx=2)
        
        disasm_group = debug_frame(group1, debug_name="Disasm Button Group", bg=ModernStyle.BG_SECONDARY)
        disasm_group.pack(side=tk.TOP)
        
        debug_button(disasm_group, debug_name="Assembly", text="Assembly", command=lambda: self.convert_format('assembly')).grid(row=0, column=0, padx=1)
        debug_button(disasm_group, debug_name="Advanced", text="Advanced", command=lambda: self.convert_format('advanced_disasm')).grid(row=0, column=1, padx=1)
        debug_button(disasm_group, debug_name="Improved", text="Improved", command=lambda: self.convert_format('improved_disasm')).grid(row=0, column=2, padx=1)
        debug_button(disasm_group, debug_name="py65 Pro", text="py65 Pro", command=lambda: self.convert_format('py65_pro')).grid(row=1, column=0, padx=1)
        
        # Gelişmiş grubu - C ve QBasic butonları PDSX'in sağına taşındı
        ...
        debug_label(group3, debug_name="Output Formats Label", text="Output Formats:", bg=ModernStyle.BG_SECONDARY,
                fg=ModernStyle.FG_ACCENT, font=("Arial", 9, "bold")).pack(side=tk.TOP, pady=1)
        
        output_group = debug_frame(group3, debug_name="Output Button Group", bg=ModernStyle.BG_SECONDARY)
        output_group.pack(side=tk.TOP)
        
        debug_button(output_group, debug_name="PDSX", text="PDSX", command=lambda: self.convert_format('pdsx')).grid(row=0, column=0, padx=1)
        debug_button(output_group, debug_name="C", text="C", command=lambda: self.convert_format('c')).grid(row=0, column=1, padx=1)
        debug_button(output_group, debug_name="QBasic", text="QBasic", command=lambda: self.convert_format('qbasic')).grid(row=0, column=2, padx=1)
        debug_button(output_group, debug_name="Pseudo", text="Pseudo", command=lambda: self.convert_format('pseudo')).grid(row=1, column=0, padx=1)
        debug_button(output_group, debug_name="Py65", text="Py65", command=lambda: self.convert_format('py65')).grid(row=1, column=1, padx=1)
        
        # 🍎 Enhanced BASIC Transpiler Grubu - Decompiler'dan taşındı
        ...
        transpiler_group = debug_frame(transpiler_frame, debug_name="Transpiler Button Group", bg=ModernStyle.BG_SECONDARY)
        transpiler_group.pack(side=tk.TOP)
        
        # Enhanced BASIC Transpiler butonları
        if EnhancedBasicDecompiler:
            debug_button(transpiler_group, debug_name="Enhanced BASIC", text="🔄 Enhanced BASIC", command=lambda: self.convert_format('enhanced_basic')).grid(row=0, column=0, padx=1)
            debug_button(transpiler_group, debug_name="QBasic Transpile", text="🎯 → QBasic", command=lambda: self.convert_format('transpile_qbasic')).grid(row=0, column=1, padx=1)
            debug_button(transpiler_group, debug_name="C Transpile", text="⚙️ → C", command=lambda: self.convert_format('transpile_c')).grid(row=0, column=2, padx=1)
            debug_button(transpiler_group, debug_name="CPP Transpile", text="🔧 → C++", command=lambda: self.convert_format('transpile_cpp')).grid(row=1, column=0, padx=1)
            debug_button(transpiler_group, debug_name="PDSX Transpile", text="📝 → PDSX", command=lambda: self.convert_format('transpile_pdsx')).grid(row=1, column=1, padx=1)
            debug_button(transpiler_group, debug_name="Python Transpile", text="🐍 → Python", command=lambda: self.convert_format('transpile_python')).grid(row=1, column=2, padx=1)
        else:
            debug_label(transpiler_group, debug_name="Missing Decompiler", text="❌ Enhanced BASIC Decompiler\nbulunamadı", 
...
class DecompilerPanel(tk.Frame):
    ...
    def setup_ui(self):
        ...
        # Decompiler grubu
        group4 = tk.Frame(decompiler_frame, bg=ModernStyle.BG_SECONDARY)
...
class ExternalToolsWindow:
    ...
    def setup_ui(self):
        ...
        ttk.Button(buttons_frame, text="📁 Open with CrossViper IDE", 
                  command=self.open_with_crossviper).pack(side=tk.LEFT, padx=5)
...
class D64ConverterGUI:
    ...
    def setup_components(self):
        ...
        try:
            ...
            directory_frame = tk.LabelFrame(self.main_frame, text="", 
                                           bg=ModernStyle.BG_SECONDARY, fg=ModernStyle.FG_PRIMARY,
                                           font=("Arial", 10, "bold"))
            ...
            disassembly_frame = tk.Frame(self.main_frame, bg=ModernStyle.BG_SECONDARY)
        ...
        decompiler_frame = tk.Frame(self.main_frame, bg=ModernStyle.BG_SECONDARY)
    ...
    def setup_menu(self):
        ...
        external_tools_menu.add_command(label="📁 Open with CrossViper IDE", command=self.open_with_crossviper)
