...
# C64 GUI Engine Import
try:
    from c64_gui_engine import UniversalC64Engine
    C64_ENGINE_AVAILABLE = True
    print("✅ C64 GUI Engine yüklendi")
except ImportError:
    C64_ENGINE_AVAILABLE = False
    print("⚠️ C64 GUI Engine bulunamadı")

# LibX GUI Import - Python 3.13 uyumlu versiyon
try:
    from libx_guiX import LibXGuiX as LibXGui
    LIBX_GUI_AVAILABLE = True
    print("✅ LibX GUI X (Python 3.13) yüklendi")
except ImportError:
    try:
        from libx_gui import LibXGui
        LIBX_GUI_AVAILABLE = True
        print("✅ LibX GUI (fallback) yüklendi")
    except ImportError:
        LIBX_GUI_AVAILABLE = False
        print("⚠️ LibX GUI bulunamadı")
...
class ModuleSystem:
    ...
    def _execute_event(self, command, scope_name=None):
        ...
        if command.upper().startswith("EVENT "):
            ...
            if len(parts) >= 2:
                event_name = parts[1]
                # Event zaten tanımlı mı kontrol et
                if hasattr(self, 'event_system') and self.event_system:
                    if event_name in self.event_system.events:
                        # Event'i çalıştır
                        try:
                            self.event_system.trigger_event(event_name)
                            return True
                        except Exception as e:
                            self.logger.warning(f"Event çalıştırma hatası: {e}")
                            return False
                    else:
                        self.logger.warning(f"Event tanımlı değil: {event_name}")
                        return False
                else:
                    self.logger.warning("Event sistemi başlatılmamış")
                    return False
            else:
                self.logger.warning("EVENT komutu eksik parametre")
                return False
        elif command.upper() == "END EVENT":
            # Event bloğu sonu - normalde parser'da işlenir
            return True
        else:
            self.logger.warning(f"Bilinmeyen event komutu: {command}")
            return False
...
class pdsXInterpreter:
    ...
    def __init__(self):
        ...
        # C64 Graphics Engine - Ana dizinden import et
        try:
            if C64_ENGINE_AVAILABLE:
                self.c64_engine = UniversalC64Engine()
            else:
                raise ImportError("C64 Engine not available")
        except ImportError as e:
            print(f"C64 Engine yüklenemedi: {e}")
            self.c64_engine = None
        
        # LibX GUI System - Ana dizinden import et
        try:
            if LIBX_GUI_AVAILABLE:
                self.libx_gui = LibXGui()
            else:
                raise ImportError("LibX GUI not available")
        except ImportError as e:
            print(f"LibX GUI sistemi yüklenemedi: {e}")
            self.libx_gui = None
    ...
    def _init_function_table(self):
        ...
        funcs = {
            ...
            # C64 Memory Functions
            "PEEK": self._peek_memory,
            "POKE": self._poke_memory,
    ...
    def execute_command(self, command, scope_name=None):
        ...
        try:
            ...
            # Timing and delay commands
            elif command_upper.startswith("WAIT"):
                return self._execute_wait(command, scope_name)
            ...
            # C64 Graphics Engine Commands
            elif command_upper.startswith(("C64_INIT", "C64_CLEAR", "C64_PIXEL", "C64_CHAR", "SPRITE", "SID", "SCREEN", "CHRSET", "POKE", "PEEK")):
                return self._execute_c64_command(command, scope_name)
            
            # LibX GUI Commands
            elif command_upper.startswith(("WINDOW", "BUTTON", "LABEL", "ENTRY", "INPUT", "CANVAS", "TEXT", "LISTBOX", "FRAME", "MENU", "SHOW", "HIDE", "CLOSE", "BIND", "THEME", "RUN", "MSGBOX", "FILEDIALOG")):
                return self._execute_gui_command(command, scope_name)
    ...
    def _execute_c64_command(self, command, scope_name=None):
        """C64 Graphics Engine komutlarını çalıştır"""
        # Debug mesajını kaldırdık - sadece gerektiğinde göster
        # print(f"🎮 C64 Command Executed: {command}")
        
        if not C64_ENGINE_AVAILABLE:
            print("⚠️ C64 Engine yüklenmedi - basit C64 simülasyonu")
            return None  # Boolean True yerine None döndür
        
        try:
            command = command.strip()
            command_upper = command.upper()
            
            # C64 Engine başlat (lazy loading)
            if not hasattr(self, 'c64_engine') or not self.c64_engine:
                try:
                    print("🔧 C64 Engine başlatılıyor...")
                    self.c64_engine = UniversalC64Engine()
                    print("✅ C64 Engine başlatıldı")
                except Exception as e:
                    print(f"❌ C64 Engine başlatma hatası: {e}")
                    import traceback
                    traceback.print_exc()
                    return None  # Boolean True yerine None döndür
            
            # SPRITE komutları
            if command_upper.startswith("SPRITE"):
                try:
                    print(f"🚀 SPRITE komutu işleniyor: {command}")
                    result = self.c64_engine.execute_command(command)
                    print(f"✅ C64 SPRITE sonuç: {result}")
                    return None  # C64 engine result'u ignore et - her zaman None döndür
                except Exception as e:
                    print(f"❌ C64 SPRITE hatası: {e}")
                    import traceback
                    traceback.print_exc()
                    return None  # Error için de None döndür
            
            # SID Audio komutları  
            elif command_upper.startswith("SID"):
                try:
                    result = self.c64_engine.execute_command(command)
                    if result:
                        print(f"C64 SID: {result}")
                    return None  # C64 engine result'u ignore et - her zaman None döndür
                except Exception as e:
                    print(f"C64 SID hatası: {e}")
                    return None  # Boolean True yerine None döndür
            ...
            # SCREEN komutları
                ...
                try:
                    result = self.c64_engine.execute_command(command)
                    if result:
                        print(f"C64 SCREEN: {result}")
                    return None  # C64 engine result'u ignore et - her zaman None döndür
                except Exception as e:
                    print(f"C64 SCREEN hatası: {e}")
                    return None  # Boolean True yerine None döndür
            
            # CHRSET komutları
            elif command_upper.startswith("CHRSET"):
                try:
                    result = self.c64_engine.execute_command(command)
                    if result:
                        print(f"C64 CHRSET: {result}")
                    return None  # C64 engine result'u ignore et - her zaman None döndür
                except Exception as e:
                    print(f"C64 CHRSET hatası: {e}")
                    return None  # Boolean True yerine None döndür
            
            # Tüm diğer C64 komutları (C64_INIT, C64_CLEAR, POKE, PEEK, vb.)
            else:
                try:
                    result = self.c64_engine.execute_command(command)
                    if result:
                        print(f"✅ C64 sonuç: {result}")
                    return None  # C64 engine result'u ignore et - her zaman None döndür
                except Exception as e:
            ...
            print(f"C64 Engine hatası: {str(e)}")
            return None  # Boolean True yerine None döndür

    def _peek_memory(self, address):
        """C64 memory peek operation"""
        try:
            # C64 specific memory locations
            if address == 53269:  # VIC-II register
                return 255  # Simulated value
            elif address == 53280:  # Border color
                return 0
            elif address == 53281:  # Background color
                return 0
            else:
                # Generic memory read
                return self.memory_manager.read(address) if hasattr(self.memory_manager, 'read') else 0
        except:
            return 0
            
    def _poke_memory(self, address, value):
        """C64 memory poke operation"""
        try:
            # C64 specific memory locations
            if address in [53280, 53281]:  # Color registers
                # Handle color change
                pass
            if hasattr(self.memory_manager, 'write'):
                self.memory_manager.write(address, value)
            return None
        except:
            return None

    def _execute_gui_command(self, command, scope_name=None):
        """LibX GUI komutlarını çalıştır"""
        print(f"GUI Command Executed: {command}")
        
        if not LIBX_GUI_AVAILABLE:
            print("LibX GUI yüklenmedi - GUI komutları simüle ediliyor")
            return None  # Boolean True yerine None döndür
        ...
        try:
            ...
            # LibX GUI başlat (lazy loading)
            if not hasattr(self, 'gui_engine') or not self.gui_engine:
                try:
                    self.gui_engine = LibXGui(self)
                    print("LibX GUI Engine başlatıldı")
                except Exception as e:
                    print(f"LibX GUI başlatma hatası: {e}")
                    return None  # Boolean True yerine None döndür
            
            # GUI komutunu parse et ve çalıştır
            try:
                result = self.gui_engine.parse_gui_command(command)
                if result:
                    print(f"GUI Command Success: {command}")
                return None  # Success için None döndür
            except Exception as e:
                print(f"GUI Command Error: {e}")
                return None  # Error için de None döndür
                
        except Exception as e:
            print(f"GUI Engine hatası: {str(e)}")
            return None  # Boolean True yerine None döndür

    def _execute_wait(self, command, scope_name=None):
        """WAIT komutunu çalıştır - milisaniye bekle"""
        import time
        
        try:
            # WAIT 500 (500 milisaniye bekle)
            parts = command.split()
            if len(parts) >= 2:
                wait_time = self.evaluate_expression(parts[1], scope_name)
                wait_seconds = float(wait_time) / 1000.0  # Milisaniyeyi saniyeye çevir
                print(f"WAIT {wait_time}ms ({wait_seconds}s)")
                time.sleep(wait_seconds)
                return None  # Boolean True yerine None döndür
            else:
                # Varsayılan 100ms bekle
                time.sleep(0.1)
                return None  # Boolean True yerine None döndür
        except Exception as e:
            print(f"WAIT komut hatası: {e}")
            return None  # Boolean True yerine None döndür
