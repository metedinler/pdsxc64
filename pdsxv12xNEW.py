...
class pdsXInterpreter:
        ...
        while i < len(lines):
            ...
            try:
                ...
                # Field definitions for TYPE/STRUCT/UNION
                    ...
                    # Support both simple fields and array fields
                    # Examples: name AS STRING, weights[3] AS DOUBLE, matrix[10,10] AS INTEGER
                    match = re.match(r"(\w+)(?:\[([^\]]+)\])?\s+AS\s+(\w+)", line, re.IGNORECASE)
                    if match:
                        field_name, dimensions, field_type = match.groups()
                        if dimensions:
                            # Array field with dimensions
                            type_fields[current_type].append((field_name, f"ARRAY[{dimensions}] OF {field_type}"))
                        else:
                            # Simple field
                ...
                # IMPORT statements
                    ...
                    # Support both quoted and unquoted imports
                    match = re.match(r"IMPORT\s+[\"'](.+?)[\"'](?:\s+AS\s+(\w+))?", line, re.IGNORECASE)
                    if not match:
                        match = re.match(r"IMPORT\s+(\w+)(?:\s+AS\s+(\w+))?", line, re.IGNORECASE)
                ...
                # Regular program statements
                    ...
                    # Skip comments and empty lines here too
                    if not line.strip() or line.strip().startswith("#") or line.strip().startswith("'") or line.strip().startswith("//") or line.strip().upper().startswith("REM"):
                        pass  # Skip comments
                    else:
                        self.program.append(line)
                        self.modules[module_name]["program"].append(line)
    ...
    def execute_command(self, command, scope_name=None):
        ...
        if not command or command.startswith("'") or command.startswith("//") or command.startswith("#") or command.upper().startswith("REM"):
            return None
    ...
    def _execute_print(self, command, scope_name=None):
        """PRINT komutunu çalıştırma - BASIC syntax desteği"""
        ...
        # Advanced tokenization - hem virgül hem de semicolon desteği
        parts = []
        separators = []
        current_part = ""
        in_quotes = False
        ...
        for i, part in enumerate(parts):
            ...
            if part:
                try:
                    ...
                    output_str += f"[ERROR:{part}]"
            
            # Add separator if not last part
            if i < len(separators):
                if separators[i] == ',':
                    output_str += "\t"  # Tab for comma
                elif separators[i] == ';':
                    pass  # No space for semicolon
    ...
    def _execute_database_operations(self, command, scope_name=None):
        """Database operations - DB_CONNECT, DB_QUERY, DB_CLOSE"""
        command_upper = command.upper()
        
        if command_upper.startswith("DB_CONNECT"):
            # DB_CONNECT "connection_name" TO "database_path"
            match = re.match(r'DB_CONNECT\s+"([^"]+)"\s+TO\s+"([^"]+)"', command, re.IGNORECASE)
            if match:
                conn_name, db_path = match.groups()
                try:
                    import sqlite3
                    conn = sqlite3.connect(db_path)
                    self.db_connections[conn_name] = conn
                    print(f"Database connection '{conn_name}' established to {db_path}")
                    return None
                except Exception as e:
                    raise PdsXRuntimeError(f"Database connection failed: {e}")
            else:
                raise PdsXSyntaxError(f"Invalid DB_CONNECT syntax: {command}")
        
        elif command_upper.startswith("DB_QUERY"):
            # DB_QUERY "connection_name" SQL "SELECT * FROM table"
            match = re.match(r'DB_QUERY\s+"([^"]+)"\s+SQL\s+"([^"]+)"', command, re.IGNORECASE)
            if match:
                conn_name, sql_query = match.groups()
                if conn_name not in self.db_connections:
                    raise PdsXRuntimeError(f"Database connection '{conn_name}' not found")
                
                try:
                    conn = self.db_connections[conn_name]
                    cursor = conn.cursor()
                    cursor.execute(sql_query)
                    
                    if sql_query.strip().upper().startswith("SELECT"):
                        results = cursor.fetchall()
                        # Store results in a variable or return them
                        self.current_scope()["_DB_RESULTS"] = results
                    ...
                    raise PdsXRuntimeError(f"Database query failed: {e}")
            else:
                raise PdsXSyntaxError(f"Invalid DB_QUERY syntax: {command}")
        
        elif command_upper.startswith("DB_CLOSE"):
            # DB_CLOSE "connection_name"
            match = re.match(r'DB_CLOSE\s+"([^"]+)"', command, re.IGNORECASE)
            if match:
                conn_name = match.groups()[0]
                if conn_name in self.db_connections:
                    self.db_connections[conn_name].close()
                    del self.db_connections[conn_name]
                    print(f"Database connection '{conn_name}' closed")
                else:
                    raise PdsXRuntimeError(f"Database connection '{conn_name}' not found")
                return None
            else:
                raise PdsXSyntaxError(f"Invalid DB_CLOSE syntax: {command}")
        
        else:
            raise PdsXSyntaxError(f"Unknown database operation: {command}")
    ...
    def _evaluate_complex_expression(self, expr, scope_name=None):
        ...
        # Create namespace for evaluation with case-insensitive variable lookup
        namespace = {}
        
        # Add variables from both scopes with uppercase keys for case-insensitive lookup
        for var_name, value in self.global_vars.items():
            namespace[var_name] = value
            namespace[var_name.upper()] = value
        ...
        try:
            # Convert variable names in expression to uppercase for consistent lookup
            expr_upper = expr
            for var_name in self.global_vars.keys():
                if var_name != var_name.upper():
                    expr_upper = re.sub(r'\b' + re.escape(var_name) + r'\b', var_name.upper(), expr_upper, flags=re.IGNORECASE)
            for var_name in self.current_scope().keys():
                if var_name != var_name.upper():
                    expr_upper = re.sub(r'\b' + re.escape(var_name) + r'\b', var_name.upper(), expr_upper, flags=re.IGNORECASE)
            
            # Use AST for safe evaluation
            tree = ast.parse(expr_upper, mode='eval')
    ...
    def _get_variable(self, name):
        """Değişken değerini alma - Case insensitive"""
        ...
        # Try case-insensitive match in current scope
        for var_name, value in self.current_scope().items():
            if var_name.upper() == name.upper():
                return value
        
        # Try case-insensitive match in global scope
        for var_name, value in self.global_vars.items():
            if var_name.upper() == name.upper():
                return value
    ...
    def run(self, code=None):
        ...
        try:
            while self.running and self.program_counter < len(self.program):
                ...
                scope_name = "main"  # Default scope
                # print(f"DEBUG: Executing command {self.program_counter}: '{command}'")
    ...
    def _initialize_libx_gui(self):
        ...
        try:
            # LibX GUI import et
            from libx_guiX import LibXGuiX as LibXGui
    ...
    def _execute_c64_gui_operations(self, command, scope_name=None):
        ...
        try:
            ...
            if not hasattr(self, 'gui_initialized'):
                self.gui_initialized = False
            ...
            if command_upper.startswith("C64_INIT"):
                ...
                return self._c64_poke(command)
            elif command_upper.startswith("PEEK"):
                return self._c64_peek(command)
            elif command_upper.startswith("SID_"):
                return self._c64_sid_operations(command)
            elif command_upper.startswith("CHARSET"):
                return self._c64_charset_operations(command)
            elif command_upper.startswith("SCREEN_"):
                return self._c64_screen_operations(command)
            else:
                raise PdsXSyntaxError(f"Bilinmeyen C64 GUI komutu: {command}")
                
        except Exception as e:
            raise PdsXRuntimeError(f"C64 GUI operasyon hatası: {e}")

    def _initialize_c64_gui(self):
        """C64 GUI sistemini başlat"""
        try:
            from c64_gui_engine import get_c64_gui_engine
            self.c64_gui_engine = get_c64_gui_engine()
            self.c64_gui_engine.initialize()
            self.gui_initialized = True
            print("C64 GUI Engine başlatıldı!")
        except ImportError:
            print("C64 GUI Engine modülü bulunamadı, simülasyon modu")
            self.c64_gui_engine = None
            self.gui_initialized = True
        except Exception as e:
    ...
    def _c64_init_screen(self, command):
        """C64_INIT komutu - ekranı başlat"""
        try:
            match = re.match(r"C64_INIT\s+(\d+)", command, re.IGNORECASE)
            if match:
                mode = int(match.group(1))
            else:
                mode = 0  # Default text mode
            
            if self.c64_gui_engine:
                self.c64_gui_engine.initialize()
                # Mode'u ayrı bir komutla set et
                if mode != 0:
                    self.c64_gui_engine.memory.display_mode = mode
            
            print(f"C64 ekran başlatıldı - Mode: {mode}")
            return None
        except Exception as e:
            print(f"C64_INIT hatası: {e}")
            return None

    def _c64_clear_screen(self, command):
        """C64_CLEAR komutu - ekranı temizle"""
        try:
            match = re.match(r"C64_CLEAR\s+(\d+)", command, re.IGNORECASE)
            if match:
                color = int(match.group(1))
            else:
                color = 6  # Default blue
            
            if self.c64_gui_engine:
                self.c64_gui_engine.clear_screen(color)
            
            print(f"C64 ekran temizlendi - Renk: {color}")
            return None
        except Exception as e:
            print(f"C64_CLEAR hatası: {e}")
            return None

    def _c64_set_pixel(self, command):
        """C64_PIXEL komutu - pixel ayarla"""
        try:
            match = re.match(r"C64_PIXEL\s+(\d+)\s+(\d+)\s+(\d+)", command, re.IGNORECASE)
            if not match:
                raise PdsXSyntaxError(f"Geçersiz C64_PIXEL sözdizimi: {command}")
            
            x, y, color = map(int, match.groups())
            
            if self.c64_gui_engine:
                self.c64_gui_engine.set_pixel(x, y, color)
            
            return None
        except Exception as e:
            print(f"C64_PIXEL hatası: {e}")
            return None

    def _c64_draw_char(self, command):
        """C64_CHAR komutu - karakter çiz"""
        try:
            match = re.match(r"C64_CHAR\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)", command, re.IGNORECASE)
            if not match:
                raise PdsXSyntaxError(f"Geçersiz C64_CHAR sözdizimi: {command}")
            
            x, y, char, color = map(int, match.groups())
            
            if self.c64_gui_engine:
                self.c64_gui_engine.draw_char(x, y, char, color)
            
            return None
        except Exception as e:
            print(f"C64_CHAR hatası: {e}")
            return None

    def _c64_screen_operations(self, command):
        """SCREEN_ komutları - ekran işlemleri"""
        try:
            command_upper = command.upper()
            
            if command_upper.startswith("SCREEN_MODE"):
                match = re.match(r"SCREEN_MODE\s+(\d+)", command, re.IGNORECASE)
                if match:
                    mode = int(match.group(1))
                    if self.c64_gui_engine:
                        self.c64_gui_engine.set_screen_mode(mode)
                    print(f"Screen mode: {mode}")
                
            elif command_upper.startswith("SCREEN_COLOR"):
                match = re.match(r"SCREEN_COLOR\s+(\d+)\s+(\d+)", command, re.IGNORECASE)
                if match:
                    bg, border = map(int, match.groups())
                    if self.c64_gui_engine:
                        self.c64_gui_engine.set_screen_color(bg, border)
                    print(f"Screen colors: BG={bg}, Border={border}")
                
            elif command_upper.startswith("SCREEN_TEXT"):
                match = re.match(r"SCREEN_TEXT\s+(\d+)\s+(\d+)\s+\"([^\"]+)\"\s+(\d+)", command, re.IGNORECASE)
                if not match:
                    match = re.match(r"SCREEN_TEXT\s+(\d+)\s+(\d+)\s+([^\s]+)\s+(\d+)", command, re.IGNORECASE)
                if match:
                    x, y, text, color = match.groups()
                    x, y, color = int(x), int(y), int(color)
                    text = text.strip('"\'')
                    
                    if self.c64_gui_engine:
                        self.c64_gui_engine.draw_text(x, y, text, color)
                    print(f"Screen text: ({x},{y}) '{text}' color={color}")
                
            elif command_upper.startswith("SCREEN_SCROLL"):
                match = re.match(r"SCREEN_SCROLL\s+(\w+)\s*(\d+)?", command, re.IGNORECASE)
                if match:
                    direction, lines = match.groups()
                    lines = int(lines) if lines else 1
                    if self.c64_gui_engine:
                        self.c64_gui_engine.scroll_screen(direction.upper(), lines)
                    print(f"Screen scroll: {direction} {lines} lines")
            
            return None
        except Exception as e:
            print(f"SCREEN_ komut hatası: {e}")
            return None
    ...
    def _c64_sprite_operations(self, command):
        ...
        try:
            ...
            if command_upper.startswith("SPRITE LOAD"):
                match = re.match(r"SPRITE\s+LOAD\s+(\d+)\s+\"([^\"]+)\"", command, re.IGNORECASE)
                if not match:
                    match = re.match(r"SPRITE\s+LOAD\s+(\d+)\s+([^\s]+)", command, re.IGNORECASE)
                if match:
                    sprite_id, filepath = match.groups()
                    sprite_id = int(sprite_id)
                    filepath = filepath.strip('"\'')
                    
                    if self.c64_gui_engine:
                        success = self.c64_gui_engine.load_sprite(sprite_id, filepath)
                    else:
                        success = True  # Simülasyon
                    
                    print(f"Sprite {sprite_id} yükle: {filepath} - {'Başarılı' if success else 'Başarısız'}")
                
            elif command_upper.startswith("SPRITE ENABLE"):
                match = re.match(r"SPRITE\s+ENABLE\s+(\d+)", command, re.IGNORECASE)
                if match:
                    sprite_id = int(match.group(1))
                    if self.c64_gui_engine:
                        self.c64_gui_engine.enable_sprite(sprite_id)
                    print(f"Sprite {sprite_id} etkinleştirildi")
                
            elif command_upper.startswith("SPRITE DISABLE"):
                match = re.match(r"SPRITE\s+DISABLE\s+(\d+)", command, re.IGNORECASE)
                if match:
                    sprite_id = int(match.group(1))
                    if self.c64_gui_engine:
                        self.c64_gui_engine.disable_sprite(sprite_id)
                    print(f"Sprite {sprite_id} devre dışı")
                
            elif command_upper.startswith("SPRITE MOVE"):
                match = re.match(r"SPRITE\s+MOVE\s+(\d+)\s+(\d+)\s+(\d+)", command, re.IGNORECASE)
                if match:
                    sprite_id, x, y = map(int, match.groups())
                    if self.c64_gui_engine:
                        self.c64_gui_engine.move_sprite(sprite_id, x, y)
                    print(f"Sprite {sprite_id} hareket: ({x}, {y})")
                
            elif command_upper.startswith("SPRITE COLOR"):
                match = re.match(r"SPRITE\s+COLOR\s+(\d+)\s+(\d+)", command, re.IGNORECASE)
                if match:
                    sprite_id, color = map(int, match.groups())
                    if self.c64_gui_engine:
                        self.c64_gui_engine.set_sprite_color(sprite_id, color)
                    print(f"Sprite {sprite_id} renk: {color}")
            
            return None
        except Exception as e:
            print(f"SPRITE komut hatası: {e}")
            return None

    def _c64_poke(self, command):
        """POKE komutu - bellek yazma"""
        try:
            match = re.match(r"POKE\s+(\d+)\s+(\d+)", command, re.IGNORECASE)
            if not match:
                raise PdsXSyntaxError(f"Geçersiz POKE sözdizimi: {command}")
            ...
            print(f"POKE hatası: {e}")
            return None

    def _c64_peek(self, command):
        """PEEK komutu - bellek okuma"""
        try:
            match = re.match(r"PEEK\s*\(\s*(\d+)\s*\)", command, re.IGNORECASE)
            if not match:
                raise PdsXSyntaxError(f"Geçersiz PEEK sözdizimi: {command}")
            ...
            print(f"PEEK hatası: {e}")
            return 0

    def _c64_sid_operations(self, command):
        """SID_ komutları - ses sistemi"""
        try:
            command_upper = command.upper()
            
            if command_upper.startswith("SID_LOAD"):
                match = re.match(r"SID_LOAD\s+\"([^\"]+)\"", command, re.IGNORECASE)
                if not match:
                    match = re.match(r"SID_LOAD\s+([^\s]+)", command, re.IGNORECASE)
                if match:
                    filepath = match.group(1).strip('"\'')
                    if self.c64_gui_engine:
                        success = self.c64_gui_engine.load_sid(filepath)
                    else:
                        success = True  # Simülasyon
                    print(f"SID yükle: {filepath} - {'Başarılı' if success else 'Başarısız'}")
                
            elif command_upper.startswith("SID_PLAY"):
                match = re.match(r"SID_PLAY\s+(\d+)", command, re.IGNORECASE)
                if match:
                ...
                if self.c64_gui_engine:
                    self.c64_gui_engine.play_sid(voice)
                print(f"SID çal - Voice: {voice}")
                
            elif command_upper.startswith("SID_STOP"):
                match = re.match(r"SID_STOP\s+(\d+)", command, re.IGNORECASE)
                if match:
                    voice = int(match.group(1))
                else:
                    voice = 0
                ...
                if self.c64_gui_engine:
                    self.c64_gui_engine.stop_sid(voice)
                print(f"SID durdur - Voice: {voice}")
                
            elif command_upper.startswith("SID_VOLUME"):
                match = re.match(r"SID_VOLUME\s+([\d.]+)", command, re.IGNORECASE)
                if match:
                    volume = float(match.group(1))
                    if self.c64_gui_engine:
                        self.c64_gui_engine.set_sid_volume(volume)
                    print(f"SID volume: {volume}")
            
            return None
        except Exception as e:
            print(f"SID komut hatası: {e}")
            return None

    def _c64_charset_operations(self, command):
        """CHARSET komutları - karakter seti"""
        try:
            command_upper = command.upper()
            
            if command_upper.startswith("CHARSET LOAD"):
                match = re.match(r"CHARSET\s+LOAD\s+(\d+)\s+\"([^\"]+)\"", command, re.IGNORECASE)
                if not match:
                    match = re.match(r"CHARSET\s+LOAD\s+(\d+)\s+([^\s]+)", command, re.IGNORECASE)
                if match:
                    charset_id, filepath = match.groups()
                    charset_id = int(charset_id)
                    filepath = filepath.strip('"\'')
                    
                    if self.c64_gui_engine:
                        success = self.c64_gui_engine.load_charset(charset_id, filepath)
                    else:
                        success = True  # Simülasyon
                    
                    print(f"Charset {charset_id} yükle: {filepath} - {'Başarılı' if success else 'Başarısız'}")
                
            elif command_upper.startswith("CHARSET SET"):
                match = re.match(r"CHARSET\s+SET\s+(\d+)", command, re.IGNORECASE)
                if match:
                    charset_id = int(match.group(1))
                    if self.c64_gui_engine:
                        self.c64_gui_engine.set_charset(charset_id)
                    print(f"Aktif charset: {charset_id}")
            
            return None
        except Exception as e:
            print(f"CHARSET komut hatası: {e}")
            return None
...
def main():
    ...
    if args.repl or not args.file:
        # REPL mode
        print("REPL modu şu an desteklenmiyor")
        return
    else:
        # File execution mode
        ...
        # C64 GUI motoru başlatıldıysa, ana döngüyü çalıştır
        if interpreter.c64_gui_engine and interpreter.gui_initialized:
            print("C64 GUI ana döngüsü başlatılıyor...")
            interpreter.c64_gui_engine.start_main_loop()
