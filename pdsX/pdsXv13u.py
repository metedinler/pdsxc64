...
from gui_libx import GuiLibX
from async_manager import AsyncManager
from lowlevel_engine import InlineASM, InlineC, UnsafeMemoryManager, SysCallWrapper
from bytecode_compiler import BytecodeCompiler
from dll_manager import DllManager, ApiManager
...
def install_missing_libraries():
    required = {
        ...
        'packaging': 'packaging', 'sympy': 'sympy', 'tkinter': 'tkinter',
        'numba': 'numba'
    }
...
class PdsXv13:
    def __init__(self):
        ...
        self.type_table = {
            ...
            "FLOAT128": float, "FLOAT256": float, "STRING8": str, "STRING16": str,
            "YAPI": dict
        }
        ...
        self.function_table = {
            ...
            "COUNT": lambda cls: sum(1 for obj in globals().values() if isinstance(obj, cls))
        }
        ...
        while i < len(lines):
            ...
            if line_upper.startswith("SUB "):
                ...
                if match:
                    ...
                    raise PdsXException(f"TYPE tanımı hatası: {line}")
                i += 1
                ...
                if match:
                    ...
                    raise PdsXException("ABSTRACT CLASS komutunda sözdizimi hatası")
                ...
                if match:
                    ...
                    raise PdsXException(f"{method_type} tanımı hatası: {line}")
                ...
                if match:
                    ...
                    raise PdsXException("STATIC komutunda sözdizimi hatası")
    ...
    def import_module(self, file_name, module_name=None):
        ...
        if ext not in (".basx", ".libx", ".hx", ".hz"):
            for try_ext in [".hz", ".hx"]:
                ...
                raise PdsXException("Desteklenmeyen dosya uzantısı. Uzantı .basX, .libX, .hX veya .hz olmalı")
        if not os.path.exists(file_name):
            raise PdsXException(f"Dosya bulunamadı: {file_name}")
        module_name = module_name or os.path.splitext(os.path.basename(file_name))[0]
        if module_name in self.modules:
            raise PdsXException(f"Modül zaten yüklü: {module_name}")
    ...
    def execute_method(self, method_name, method_body, params, args, scope_name):
        if len(args) != len(params):
            raise PdsXException(f"Parametre uyuşmazlığı: {method_name}")
        ...
        try:
            ...
            if command_upper.startswith("ON GOSUB"):
                ...
                if match:
                    ...
                    raise PdsXException("ON GOSUB komutunda sözdizimi hatası")

            if command_upper.startswith("~ "):
                args = command[2:].split(",")
                result = self.function_table["~"](*[self.evaluate_expression(arg.strip(), scope_name) for arg in args])
                return None

            if command_upper.startswith("?? "):
                match = re.match(r"\?\?\s+\"([^\"]+)\",\s*(\w+)", command, re.IGNORECASE)
                if match:
                    prompt, var_name = match.groups()
                    result = self.function_table["??"](prompt)
                    self.current_scope()[var_name] = result
                    return None
                else:
                    raise PdsXException("?? sözdizimi hatası")

            if command_upper.startswith("? "):
                match = re.match(r"\?\s+(.+?)\s+~\s+\"([^\"]+)\"\s+\"([^\"]+)\"\s+AS\s+(\w+)", command, re.IGNORECASE)
                if match:
                    cond, true_val, false_val, alias_name = match.groups()
                    result = self.function_table["?"](self.evaluate_expression(cond, scope_name), true_val, false_val)
                    self.current_scope()[alias_name] = result
                    return None
                else:
                    raise PdsXException("? sözdizimi hatası")
            ...
            if command_upper.startswith("LET"):
                ...
                if match:
                    ...
                    if f"_{var_name}_const" in self.current_scope():
                        raise PdsXException(f"Değişmez değişken: {var_name}")
            ...
            if command_upper.startswith("FOR"):
                ...
                if match:
                    ...
                    match = re.match(r"FOR\s+(\w+)\s*=\s*(\d+)\s+TO\s+(\d+)(?:\s+STEP\s+(\d+))?", command, re.IGNORECASE)
                    if match:
                        var_name, start, end, step = match.groups()
                        start = int(start)
                        end = int(end)
                        step = int(step) if step else 1
                        commands = command[1] if isinstance(command, tuple) else []
                        self.current_scope()[var_name] = start
                        self.loop_stack.append({
                            "start": self.program_counter,
                            "type": "FOR",
                ...
                if self.loop_stack and self.loop_stack[-1]["type"] == "FOR":
                    ...
                    if (loop_info["step"] > 0 and current_value <= loop_info["end"]) or \
                       (loop_info["step"] < 0 and current_value >= loop_info["end"]):
                        if "commands" in loop_info:
                            for cmd in loop_info["commands"]:
            ...
            if command_upper.startswith("GOTO"):
                ...
                if match:
                    ...
                    raise PdsXException("GOTO komutunda sözdizimi hatası")
            ...
            if command_upper.startswith("GOSUB"):
                ...
                if match:
                    ...
                    raise PdsXException("GOSUB komutunda sözdizimi hatası")
            ...
            if command_upper.startswith("CALL"):
                ...
                raise PdsXException("CALL komutunda sözdizimi hatası")
    ...
    def repl(self):
        self.repl_mode = True
        print("pdsXv13 REPL - Çıkış için 'EXIT'")
        while True:
            try:
                cmd = input("> ")
                if cmd.upper() == "EXIT":
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='pdsXv13 Interpreter')
