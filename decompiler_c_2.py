...
class ASTNode:
    def __init__(self, node_type, children=None, value=None, label=None, params=None):
        self.type = node_type  # 'program', 'switch', 'for', 'while', 'function', 'struct', 'array', 'enum', 'assign', 'expr'
            ...
            for line in self.disassembly_lines:
                ...
                if match:
                    ...
                    if opcode == 'JSR' and isinstance(operand, int):
                        self.functions[operand] = f"func_{operand:04x}" if operand not in KERNAL_MAP else KERNAL_MAP[operand]
        ...
        try:
            ...
            if op == 'LDA':
                ...
                if operand in self.functions:
                    ...
                    if KERNAL_MAP[operand] in ['sin', 'cos']:
                        return ASTNode('math_call', value=f"float result = {KERNAL_MAP[operand]}(a);")
                    elif KERNAL_MAP[operand] in ['len', 'mid', 'left', 'right']:
                        return ASTNode('string_call', value=f"char *result = {KERNAL_MAP[operand]}(memory, a);")
                    else:
                        return ASTNode('kernal_call', value=f"{KERNAL_MAP[operand]}(a);" if KERNAL_MAP[operand] == 'CHROUT' else f"{KERNAL_MAP[operand]}();")
            elif op == 'RTS':
        ...
        try:
            ...
            for start_addr, end_addr, block in blocks:
                ...
                if pattern == 'struct':
                    ...
                    self.ast.children.append(ASTNode('array_def', value=f"uint8_t {array_name}[{target}];"))
                    for instr in block:
                    ...
                    self.ast.children.append(ASTNode('function', value=f"static inline void {macro_name}(void)", children=[self.translate_instruction(b) for b in macro_block]))
                elif pattern == 'loop':
                    init = f"int i = 1;"
                    cond = f"i <= {loop_count}"
                    incr = "i++;"
                    ...
                    self.ast.children.append(ASTNode('function', value=f"void callback_{cond_val:04x}(void)", children=[self.translate_instruction(b) for b in block]))
    ...
    def emit_code(self):
        ...
        try:
            lines = [
                ...
                'float cos(float);',
                'char* len(uint8_t*);',
                'char* mid(uint8_t*, uint8_t);',
                'char* left(uint8_t*, uint8_t);',
                'char* right(uint8_t*, uint8_t);'
            ]
            # Enum tanımları
            if self.constants:
                ...
                lines.append("};")
            # Struct, dizi ve global değişken tanımları
            for node in self.ast.children:
                if node.type in ['struct_def', 'array_def', 'global_var']:
            ...
            for node in self.ast.children:
                if node.type == 'switch':
                    ...
                    lines.append("}")
                    self.line_number += 1
                elif node.type == 'kernal_call' or node.type == 'math_call' or node.type == 'string_call':
                    lines.append(f"    {node.value}")
                    self.line_number += 1
